...
void do_coredump(siginfo_t *siginfo)
{
	...
	#ifdef VENDOR_EDIT
	//Li.Liu@EXP.Android.Stability.Crash, 2017/07/05, Add for current->thread_leader->comm , current_uid and more.
	//demo only allow system_server, surfaceflinger, mediaserver and other system process in user build.
	#ifndef CONFIG_MT_ENG_BUILD
	//printk(KERN_WARNING "current->group_leader->comm is (%s)\n", current->group_leader->comm);
	if (strncmp(current->group_leader->comm, "system_server", 16)
		&& strncmp(current->group_leader->comm, "surfaceflinger", 16)
		&& strncmp(current->group_leader->comm, "mediaserver", 16)
