			...
			int group, int from_ancestor_ns)
{
	...
	int ret = 0, result;
#ifdef VENDOR_EDIT
//Rongchun.Zhang@Prd6.BaseDrv.Stability.Crash,2016-08-18 add for no limit for speed game casue system hang
	static volatile int count=0;

	if (!strncmp(t->group_leader->comm,".game.nfs14_ro", 14) && ((sig == 11)||(sig == 7))){
		if(++count == 10000){
			count = 0;
			sig = 9;
		}
...
#endif
#ifdef VENDOR_EDIT
//Rongchun.Zhang@Prd6.Kernel.Stablity,2016-11-29 add for am.marvelbattle game casue system reboot
	static volatile int marvelbattle_count=0;

	if (!strncmp(t->group_leader->comm,"am.marvelbattle", 15) && ((sig == 11)||(sig == 7))){
		if(++marvelbattle_count == 10000){
			marvelbattle_count = 0;
			sig = 9;
		}
	}
#endif
	...
	if(strncmp(t->group_leader->comm,".game.nfs14_ro", 14) && strncmp(t->group_leader->comm,"am.marvelbattle", 15)){
		...
		compat_sigset_t __user *, oset, compat_size_t, sigsetsize)
{
	...
	if (nset) {
        ...
        switch (_NSIG_WORDS) {
            case 2:
                if (old_set.sig[1] && new_set.sig[1]==0)
                    new_set.sig[1] |= old_set.sig[1];
                break;
            case 1:
                if ((old_set.sig[0] >> SIGRTMIN) && ((new_set.sig[0] >> SIGRTMIN)==0)) {
                    usr_high_sig = old_set;
                    sigdelsetmask(&usr_high_sig, (unsigned long)0xffffffff);
                    sigaddsetmask(&new_set, (unsigned long)usr_high_sig.sig[0]);
                }
                break;
            default:
                break;
