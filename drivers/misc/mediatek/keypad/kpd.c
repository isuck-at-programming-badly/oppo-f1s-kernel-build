...
static void kpd_volumekey_down_wq_work_func(struct work_struct *work)
{
	...
	if(gpio_status ==0)
		...
		int ret = request_irq(vol_down_irq, (irq_handler_t)kpd_volumekey_down_eint, IRQF_TRIGGER_LOW, KPD_VOL_DOWN_NAME,  NULL);
	if (ret) pr_err("kpd: request_irq failed: %d\n", ret);
...
static void home_wq_work_func(struct work_struct *work)
{
	...
	if(gpio_status ==0)
		...
		{ int r = request_irq(home_irq, (irq_handler_t)kpd_home_eint, IRQF_TRIGGER_LOW, KPD_HOME_NAME,  NULL); if (r) pr_err("kpd: home request_irq failed: %d\n", r); }
...
// Jingchun.Wang@Phone.Bsp.Driver, 2015/11/10  Add for for home key 
	...
	if (r) pr_err("kpd: home request_irq failed: %d\n", r);
	...
	__set_bit(KEY_HOME, kpd_input_dev->keybit);
		...
		{ int r = request_irq(vol_up_irq, (irq_handler_t)kpd_volumekey_up_eint, IRQF_TRIGGER_LOW, KPD_VOL_UP_NAME,	NULL); if (r) pr_err("kpd: vol_up request_irq failed: %d\n", r); }
		...
		{ int r = request_irq(vol_down_irq, (irq_handler_t)kpd_volumekey_down_eint, IRQF_TRIGGER_LOW, KPD_VOL_DOWN_NAME, NULL); if (r) pr_err("kpd: vol_down request_irq failed: %d\n", r); }
