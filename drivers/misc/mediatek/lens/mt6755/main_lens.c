...
extern int kdVIOPowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, bool On, char* mode_name);
extern int kdVAFPowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, bool On, char* mode_name);
#endif
...
/*oppo hufeng 20160531 add for lens alive test*/
	case AFIOC_T_CHECKALIVE:
		if (g_pstAF_CurDrv)
		{
			kdVIOPowerOn(1, 0, true, "main_lens");
			kdVAFPowerOn(1, 0, true, "main_lens");
			msleep(10);
			i4RetValue = g_pstAF_CurDrv->pAF_Ioctl(a_pstFile, a_u4Command, a_u4Param);
			kdVAFPowerOn(1, 0, false, "main_lens");
			kdVIOPowerOn(1, 0, false, "main_lens");
		}
		break;
#endif
	default:
