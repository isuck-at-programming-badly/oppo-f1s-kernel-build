...
#define MSDC_MAX_CONTINUOUS_FAIL_REQUEST_COUNT (400)
...
#ifndef VENDOR_EDIT
// Jingchun.Wang@Phone.Bsp.Driver, 2016/04/12  Modify for remove bad card when exceed max read timeout count 
		if (host->continuous_fail_request_count <
		    MSDC_MAX_CONTINUOUS_FAIL_REQUEST_COUNT) {
#else /*VENDOR_EDIT*/
		if ((host->continuous_fail_request_count <
		    MSDC_MAX_CONTINUOUS_FAIL_REQUEST_COUNT) && 
		    (host->read_time_tune < MSDC_MAX_CONTINUOUS_FAIL_REQUEST_COUNT)) {
#endif /*VENDOR_EDIT*/
