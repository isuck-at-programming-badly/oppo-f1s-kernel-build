...
#ifdef VENDOR_EDIT
//Modified by tong.han@Bsp.Group.Tp for sd card recongnize(using ext power not VMCH),2015-12-25.
	#ifndef GPIO_TF_EN 
		#if defined(CONFIG_MTK_PMIC_CHIP_MT6353)
		...
		msdc_ldo_power(card_on, POWER_LDO_VMCH, VOL_3000,
			&g_msdc1_flash);
	#else
			mt_set_gpio_out (GPIO_TF_EN, on);
			udelay(300);
			pr_err("msdc1 setting ext_power using gpio %d state:%d\n",GPIO_TF_EN,on);
	#endif
#else
		#if defined(CONFIG_MTK_PMIC_CHIP_MT6353)
		/*  ramp-up time: 0: 40us, 1: 240us */
