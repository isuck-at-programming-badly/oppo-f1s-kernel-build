...
//MoFei@EXP.BaseDrv.charge,2015/12/22 add for smallboard match
#define  MAIN_BOARD_ID_GPIO    (GPIO16|0x80000000) 
#endif

#ifdef VENDOR_EDIT_15331
//chaoying.chen@EXP.BaseDrv.charge,2016/04/19 add for smallboard match
#define  MAIN_BOARD_ID_GPIO_1    (GPIO14|0x80000000) 
#define  MAIN_BOARD_ID_GPIO_2    (GPIO15|0x80000000) 
#define  MAIN_BOARD_ID_GPIO_3    (GPIO16|0x80000000) 
#define  MAIN_BOARD_ID_GPIO_4    (GPIO20|0x80000000) 
#endif
...
#if (defined VENDOR_EDIT_15311) || (defined VENDOR_EDIT_15331)
//MoFei@EXP.BaseDrv.charge,2016/03/08 add for smallboard match
//chaoying.chen@EXP.BaseDrv.charge,2016/04/13 add  charging current compensate for 15331
enum {
    Channel_12 = 12,
    Channel_13,
    Channel_14,
...
int get_board_volt(int times, int Channel)
{
    int ret = 0, data[4], i, ret_value = 0, ret_temp = 0;

    if( IMM_IsAdcInitReady() == 0 )
    {
	 printk("[get_board_volt] AUXADC is not ready\n");
        return 0;
    }
    i = times ;
    while (i--)
	...
	if(ret_value != 0) 
	{
		printk("[get_board_volt] error getting  Channel %d adc value\n",Channel);
		i++;
		continue;
	}
	ret += ret_temp;
	...
	ret = ret*1500/4096;
    ret = ret/times;
	printk("[get_board_volt] Channel %d: vol_ret=%d\n",Channel,ret);
	return ret;
}
#endif
...
//zhengjiang.zhu@EXP.BaseDrv,2016/01/15   sync neixiao software interface of Mobile  Operator
...
#if (defined VENDOR_EDIT_15311) || (defined VENDOR_EDIT_15331)
/*chaoying.chen@EXP.BaseDrv.charge,2016/03/08 add modem version node for 15311 */
static int oppo_modem_version = 0;
char modem_version_string[30]="ERROR";
static ssize_t oppo_modem_version_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    ...
    num_read_chars += sprintf(temp_buffer, "%d\n",oppo_modem_version);
	num_read_chars = simple_read_from_buffer(buf, count, pos, temp_buffer, strlen(temp_buffer));
	pr_info("[oppo_modem_version_read]oppo_modem_version = %d[%s]\n",oppo_modem_version,modem_version_string);	
	#ifdef VENDOR_EDIT_15331
	//chaoying.chen@EXP.BaseDrv.charge,2016/04/19 add  modem_version gpio display for 15331
	pr_info("[modem version] gpio14 = %d   gpio15 = %d  gpio16 = %d   gpio20 = %d \n",mt_get_gpio_in(MAIN_BOARD_ID_GPIO_1), 
	       mt_get_gpio_in(MAIN_BOARD_ID_GPIO_2), mt_get_gpio_in(MAIN_BOARD_ID_GPIO_3),mt_get_gpio_in(MAIN_BOARD_ID_GPIO_4));	
	#endif //VENDOR_EDIT_15331
	return num_read_chars; 
}


static ssize_t oppo_modem_version_write(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	/*not support write now*/
	return count;
}
static const struct file_operations oppo_modem_version_fops = {
	.write		= oppo_modem_version_write,
	.read		= oppo_modem_version_read,
};
#endif //VENDOR_EDIT_15311

#if (defined VENDOR_EDIT_15311) || (defined VENDOR_EDIT_15331)
/*chaoying.chen@EXP.BaseDrv.charge,2016/03/08 add export version node for 15311 */
static int export_version = 0;
static ssize_t oppo_export_version_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	int num_read_chars = 0;
       ...
       num_read_chars += sprintf(temp_buffer, "%d\n",export_version);
	num_read_chars = simple_read_from_buffer(buf, count, pos, temp_buffer, strlen(temp_buffer));
	pr_info("[oppo_export_version_read] export_version_vol = %d mv,export_version = %d[%s]\n", get_board_volt(5,Channel_14),export_version, export_version ?(export_version == 1?"export":"domestic"):"unknown");	
	return num_read_chars; 
}


static ssize_t oppo_export_version_write(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	/*not support write now*/
	return count;
}
static const struct file_operations oppo_export_version_fops = {
	.write		= oppo_export_version_write,
	.read		= oppo_export_version_read,
};
#endif //VENDOR_EDIT_15311
...
static int oppo_dev_platform_probe(struct platform_device *pdev)
{
	...
	#ifdef VENDOR_EDIT_15311
  	//MoFei@EXP.BaseDrv.charge,2016/4/14 add for smallboard match
	/*chaoying.chen@EXP.BaseDrv.charge,2016/04/14  modify smallboard match for 15311 */
	    int id0 = 0;
	    int board_vol = 0 ;
	    int export_version_vol = 0 ;
		board_vol = get_board_volt(5,Channel_13);
		id0 = mt_get_gpio_in(MAIN_BOARD_ID_GPIO);
		if(id0 == 1 && (board_vol > 150 && board_vol <= 750))
			oppo_smallboard = 1;
		if(id0 == 0 && ((board_vol >= 750 && board_vol <= 1500) || (board_vol >= 0 && board_vol <= 150)))
			oppo_smallboard = 1;
		printk("oppo_smallboard_idetify_15311,gpio_16 = %d,board_vol = %d,oppo_smallboard = %d\n",id0,board_vol,oppo_smallboard);
	#elif defined (VENDOR_EDIT_15331)
		/*chaoying.chen@EXP.BaseDrv.charge,2016/04/21  modify smallboard match for 15331 */
		int board_vol = 0 ;
		int id0 = 0, id1 = 0, id2 = 0, id3 = 0;
		board_vol = get_board_volt(5,Channel_14);
	#else
    ...
    #ifdef VENDOR_EDIT_15311
	/*chaoying.chen@EXP.BaseDrv.charge,2016/02/26 add modem version node for 15311 */
	 if( (board_vol > 0 && board_vol <= 150))
	 {
			oppo_modem_version = 1;
			strcpy(modem_version_string,"TW_SECOND_RESOUCES");
	  }		
	 else if( (board_vol > 150 && board_vol <= 450))
	 {
			oppo_modem_version = 2;
			strcpy(modem_version_string,"SKY_TW");
	 }	
	 else if( (board_vol > 450 && board_vol <= 750))
	 {
			oppo_modem_version = 3;
			strcpy(modem_version_string,"RFMD_TW");
	 }
	  else if( (board_vol >750 && board_vol <= 1050))
	 {
			oppo_modem_version = 4;
			strcpy(modem_version_string,"SKY_EU");
	 }	
	 else if((board_vol > 1050 && board_vol <= 1350))
	{
			oppo_modem_version = 5;
			strcpy(modem_version_string,"RFMD_EU");
	 }
	 else if((board_vol > 1350 && board_vol <= 1500))
	 {
			oppo_modem_version = 6;
			strcpy(modem_version_string,"EU_SECOND_RESOUCES");
	 }
	pr_info("[modem version]board_vol = %d mv,oppo_modem_version = %d[%s]\n",board_vol,oppo_modem_version,modem_version_string);	
	proc_create("oppo_modem_version", 0666, NULL, &oppo_modem_version_fops);

    /*chaoying.chen@EXP.BaseDrv.charge,2016/02/29 add export version node for 15311 */
	 export_version_vol = get_board_volt(5,Channel_14);
	 if( export_version_vol > 800 && export_version_vol <= 1000)
	 {
	   	 export_version = 1;
	  }
	 else if (export_version_vol > 350 && export_version_vol <= 550)
	  {
	   	 export_version = 2;
	  }	 
	 pr_info("[oppo export version] export_version_vol = %d mv,export_version = %d[%s]\n",export_version_vol, export_version, export_version ?(export_version == 1?"export":"domestic"):"unknown");  
	 proc_create("oppo_export_version", 0666, NULL, &oppo_export_version_fops);
	 #elif defined (VENDOR_EDIT_15331)
	 	/*chaoying.chen@EXP.BaseDrv.charge,2016/04/21  add modem version node for 15331 */
		/*MoFei@EXP.BaseDrv.charge,2016/05/11 modify for "MAS" smallboard match */
		id0 = mt_get_gpio_in(MAIN_BOARD_ID_GPIO_1);
		id1 = mt_get_gpio_in(MAIN_BOARD_ID_GPIO_2);
		id2 = mt_get_gpio_in(MAIN_BOARD_ID_GPIO_3);
		id3 = mt_get_gpio_in(MAIN_BOARD_ID_GPIO_4);

		if((id0 == 0 ) && ( id1 == 0) && ( id2 == 1)  && ( id3 == 0))
		{
			oppo_modem_version = 7;
			strcpy(modem_version_string,"SKY_TW");
			if( board_vol > 830 && board_vol < 930)
				oppo_smallboard = 1;
		}
		else if((id0 == 0 ) && ( id1 == 0) && ( id2 == 1)  && ( id3 == 1))
		{
			oppo_modem_version = 11;
			strcpy(modem_version_string,"RFMD_TW");
			if(board_vol > 830 && board_vol < 930)
				oppo_smallboard = 1;
		}
		else if((id0 == 0 ) && ( id1 == 1) && ( id2 == 0))
		{
			oppo_modem_version = 8;
			strcpy(modem_version_string,"EU");
			if(board_vol > 550 && board_vol < 650)
				oppo_smallboard = 1;
		}
		else if ((id0 == 1 ) && ( id1 == 0) && ( id2 == 0))
		{
			oppo_modem_version = 9;
			strcpy(modem_version_string,"INDIA");
			if (board_vol > 550 && board_vol < 650)
				oppo_smallboard = 1;
		}
		else if((id0 == 0 ) && ( id1 == 1) && ( id2 == 1))
		{
			oppo_modem_version = 10;
			strcpy(modem_version_string,"MAS");
			if(board_vol > 550 && board_vol < 650)
				oppo_smallboard = 1;
		}
		pr_info("oppo_smallboard_idetify_15331,board_vol = %d, modem_version_string = %s, oppo_smallboard = %d\n",board_vol,modem_version_string, oppo_smallboard);
		pr_info("[oppo modem version]oppo_modem_version = %d[%s] gpio14 = %d   gpio15 = %d  gpio16 = %d   gpio20 = %d\n",oppo_modem_version,modem_version_string,id0, id1, id2,id3);
		proc_create("oppo_modem_version", 0666, NULL, &oppo_modem_version_fops);
		export_version = 1;
		pr_info("[oppo export version]export_version = %d[%s]\n",export_version, export_version ?(export_version == 1?"export":"domestic"):"unknown");  
		proc_create("oppo_export_version", 0666, NULL, &oppo_export_version_fops);
	#else
