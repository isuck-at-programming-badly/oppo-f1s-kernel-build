...
void jpeg_drv_dec_power_off(void)
{
	...
	clk_disable_unprepare(gJpegClk.clk_venc_larb_jpgDec);
	clk_disable_unprepare(gJpegClk.clk_venc_mtcmos);
	clk_disable_unprepare(gJpegClk.clk_disp_smi_jpgDec);
...
void jpeg_drv_enc_power_on(void)
{
	...
	if (clk_prepare_enable(gJpegClk.clk_disp_smi_jpgEnc))
		JPEG_ERR("enable smi clk fail!");
...
#ifndef CONFIG_ARCH_MT6735M
	if (clk_prepare_enable(gJpegClk.clk_venc_larb_jpgEnc))
		JPEG_ERR("enable larb clk fail!");
#endif
...
#ifndef CONFIG_ARCH_MT6735M
	clk_disable_unprepare(gJpegClk.clk_venc_larb_jpgEnc);
#endif
	clk_disable_unprepare(gJpegClk.clk_venc_mtcmos);
	clk_disable_unprepare(gJpegClk.clk_disp_smi_jpgEnc);
	clk_disable_unprepare(gJpegClk.clk_disp_mtcmos);
#endif
}
...
static int jpeg_enc_ioctl(unsigned int cmd, unsigned long arg, struct file *file)
{
	...
	case JPEG_ENC_IOCTL_WAIT:
		...
		#ifdef VENDOR_EDIT
		//Yugang.Ma@EXP.GeneralApp.Camera, add for bug:813337. 2016/6/23
		if (jpeg_isr_enc_lisr() < 0) {
		    //Xubang add for jpeg encode protect
		    int ret = 0;
		    while(1)
		    {
		        ret = wait_event_interruptible_timeout(enc_wait_queue, _jpeg_enc_int_status,timeout_jiff);
		        if (ret != -ERESTARTSYS)
		            break;
                    }
                    if (ret == 0)
...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	gJpegClk.clk_disp_smi_jpgEnc = of_clk_get_by_name(node, "disp-smi-jpgenc");
	if (IS_ERR(gJpegClk.clk_disp_smi_jpgEnc))
		JPEG_ERR("get dispMTCMOS clk error!");
	...
	gJpegClk.clk_venc_larb_jpgEnc = of_clk_get_by_name(node, "venc-larb-jpgenc");
	if (IS_ERR(gJpegClk.clk_venc_larb_jpgEnc))
		JPEG_ERR("get dispMTCMOS clk error!");
...
#ifdef CONFIG_MTK_CLKMGR
#else
	gJpegClk.clk_disp_smi_jpgDec = of_clk_get_by_name(node, "disp-smi-jpgdec");
	if (IS_ERR(gJpegClk.clk_disp_smi_jpgDec))
		JPEG_ERR("get dispMTCMOS clk error!");
	gJpegClk.clk_venc_larb_jpgDec = of_clk_get_by_name(node, "venc-larb-jpgdec");
	if (IS_ERR(gJpegClk.clk_venc_larb_jpgDec))
		JPEG_ERR("get dispMTCMOS clk error!");
