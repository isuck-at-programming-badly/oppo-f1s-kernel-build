...
static int BMI160_Write_PedoParameters(struct i2c_client *client, u8 reg7A, u8 reg7B);
...
static int BMI160_ACC_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		if(pedo_enable_status||enable_status){
			GSE_LOG("acc or step counter is still working!\n");
			goto EXIT;
		}
...
#endif
#ifdef BMI160_STEP_COUNTER
	BMI160_Write_PedoParameters(client, 0x15, 0x03);
#endif
...
/*----------------------------------------------------------------------------*/
static int bmi160_acc_get_bandwidth(struct i2c_client *client, u8 *bandwidth)
{
	int comres = 0;
...
static ssize_t show_cpsbandwidth_value(struct device_driver *ddri, char *buf)
{
	...
	if (bmi160_acc_get_bandwidth(bmi160_acc_i2c_client, &bandwidth) < 0)
	{
		res = sprintf(buf, "Read error\n");
	}
	else
	{
		sprintf(strbuf, "5:12.5HZ\n8:100HZ\n9:200HZ\n12:1600HZ\nn:other\ncurrent:");
		switch(bandwidth){
			case BMI160_ACCEL_ODR_12_5HZ:
				res = sprintf(buf, "%sBMI160_ACCEL_ODR_12_5HZ\n",strbuf);
				break;
			case BMI160_ACCEL_ODR_100HZ:
				res = sprintf(buf, "%sBMI160_ACCEL_ODR_100HZ\n",strbuf);
				break;
			case BMI160_ACCEL_ODR_200HZ:
				res = sprintf(buf, "%sBMI160_ACCEL_ODR_200HZ\n",strbuf);
				break;
			case BMI160_ACCEL_ODR_1600HZ:
				res = sprintf(buf, "%sBMI160_ACCEL_ODR_1600HZ\n",strbuf);
				break;
			default:
				res = sprintf(buf, "%s%d\n",strbuf,bandwidth);
				break;
...
static DRIVER_ATTR(odr, S_IWUSR | S_IRUGO, show_cpsbandwidth_value,    store_cpsbandwidth_value);
...
static struct driver_attribute *bmi160_acc_attr_list[] = {
	...
	&driver_attr_odr,	/*g sensor bandwidth for compass tilt compensation*/
...
static ssize_t show_debounce_step_num(struct device_driver *ddri, char *buf)
{
	...
	bma_i2c_read_block(obj->client,
			BMI160_USER_STEP_COUNTER_CONF_7B_ADDR, &databuf[1], 1);
	num = databuf[1] &= 0x07;
...
static ssize_t store_debounce_step_num(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (obj == NULL)
	{
		GSE_ERR("i2c obj is null\n");
		return count;
	}
	if((num>=0)&&(num<=7)){

		comres += bma_i2c_read_block(obj->client,
			BMI160_USER_STEP_COUNTER_CONF_7B_ADDR, &databuf[1], 1);

		databuf[1] &= 0xF8;
		databuf[1] += num;
		comres += bma_i2c_write_block(obj->client,
				BMI160_USER_STEP_COUNTER_CONF_7B_ADDR, &databuf[1], 1);
		if(comres<0){
			GSE_ERR("i2c err\n");
		}
	}else{
		GSE_ERR("input is invalid:%d\n",num);
	}
...
static ssize_t show_threshold_debounce(struct device_driver *ddri, char *buf)
{
	...
	GSE_FUN();
	bma_i2c_read_block(obj->client,
			BMI160_USER_STEP_COUNTER_CONF_7A_ADDR, &reg7A, 1);
	bma_i2c_read_block(obj->client,
			BMI160_USER_STEP_COUNTER_CONF_7B_ADDR, &reg7B, 1);

	return sprintf(buf, "7A: 0x%02x 7B: 0x%02x\n",reg7A,reg7B);
}
...
static ssize_t store_threshold_debounce(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	GSE_FUN();
	sscanf(buf, "%x %x", &reg7A, &reg7B);
	databuf = (u8)reg7A;
	bma_i2c_write_block(obj->client,
				BMI160_USER_STEP_COUNTER_CONF_7A_ADDR, &databuf, 1);
	databuf = (u8)reg7B;
	bma_i2c_write_block(obj->client,
				BMI160_USER_STEP_COUNTER_CONF_7B_ADDR, &databuf, 1);
	GSE_LOG("%s: try to set reg7A:0x%02x reg7B:0x%02x\n", __func__, reg7A, reg7B);
...
/*----------------------------------------------------------------------------*/
static struct driver_attribute *bmi160_step_c_attr_list[] = {
	&driver_attr_debounce_step_num,     /*debounce step num*/
	&driver_attr_threshold_debounce,     /*debounce step num*/
};
/*----------------------------------------------------------------------------*/
static int bmi160_step_c_create_attr(struct device_driver *driver)
{
	int idx, err = 0;
	int num = (int)(sizeof(bmi160_step_c_attr_list)/sizeof(bmi160_step_c_attr_list[0]));
	if (driver == NULL)
	{
		return -EINVAL;
	}
	...
	for(idx = 0; idx < num; idx++)
	{
		if(0 != (err = driver_create_file(driver,  bmi160_step_c_attr_list[idx])))
		{
			GSE_ERR("driver_create_file (%s) = %d\n",  bmi160_step_c_attr_list[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
/*----------------------------------------------------------------------------*/
static int bmi160_step_c_delete_attr(struct device_driver *driver)
{
	int idx ,err = 0;
	int num = (int)(sizeof( bmi160_step_c_attr_list)/sizeof( bmi160_step_c_attr_list[0]));
	...
	for(idx = 0; idx < num; idx++)
	{
		driver_remove_file(driver,  bmi160_step_c_attr_list[idx]);
	}
	return err;
}


static int BMI160_acc_Enable_Pedometer_Func(struct i2c_client *client, int enable)
{
	int res = 0;
	unsigned char data[2] = {BMI160_USER_STEP_COUNTER_CONF_7B_ADDR};
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
	GSE_LOG("[%s] enable = %d\n", __func__, enable);
	...
	res = bma_i2c_read_block(client,
			BMI160_USER_STEP_COUNTER_CONF_7B_ADDR, data+1, 1);

	data[1]  = BMI160_SET_BITSLICE(data[1],
			BMI160_USER_STEP_COUNTER_EN, enable);
...
// reg7A: register 0x7A; reg7B: register 0x7B
static int BMI160_Write_PedoParameters(struct i2c_client *client, u8 reg7A, u8 reg7B)
{
	int comres = 0;
	unsigned char databuf[2] = {0};
	...
	databuf[0] = reg7A;
	comres += bma_i2c_write_block(client,
			BMI160_USER_STEP_COUNTER_CONF_7A_ADDR, &databuf[0], 1);
...
static int BMI160_Reset_Pedo_Data(struct i2c_client *client, int newValue)
{

	GSE_FUN();
	BMI160_acc_Enable_Pedometer_Func(client,true);
	return 0;
}

static int BMI160_Get_Pedo_DataReg(struct i2c_client *client, u16 *Value)
{
	int res = 0;
	u8 pedoAddr = BMI160_USER_STEP_COUNTER_DATA;
	u8 data[BMI160_STEP_COUNTER_DATA_LEN] = {0};
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
	GSE_FUN();
	...
	res = bma_i2c_read_block(client, pedoAddr, data, 2);

	*Value = (((u16)data[1])
	  << BMI160_SHIFT_8_POSITION) | data[0];
	mdelay(1);
	mutex_unlock(&obj->lock);
	
	if(res < 0)
	{
		GSE_LOG("BMI160_Get_Pedo_DataReg i2c fail!\n");
		return BMI160_ACC_ERR_I2C;
	}
	else
	{
...
static bool first_data_after_enable = false;
static int BMI160_enable_pedo(struct i2c_client *client, int enable)
{
	int res = 0;

	if (1 == enable) {

		first_data_after_enable = true;

		res = BMI160_acc_Enable_Pedometer_Func(client, enable);
...
static int bmi160_step_c_enable_nodata(int en)
{
	...
	if (value == 1) {
		if(!pedo_enable_status){
			res += BMI160_ACC_SetPowerMode(priv->client, true);
			res += BMI160_enable_pedo(priv->client, 1);
			//res += BMI160_Write_PedoParameters(priv->client, 0x15, 0x03);
			if (0 != res) {
			...
			GSE_LOG("bmi160_step_c already enable!\n");
		}
	} else {
		if(pedo_enable_status){
			pedo_enable_status = false;
			res += BMI160_enable_pedo(priv->client, 0);
			step_base_num = pre_step;
			res += BMI160_ACC_SetPowerMode(priv->client, false);
			if (0 != res) {
				GSE_LOG("BMI160_enable_pedo failed at close action!\n");
				return res;
			}
		}else{
			GSE_LOG("bmi160_step_c already disable!\n");
		}
	}
...
static int bmi160_step_c_get_data(u64 *value, int *status)
{
	...
	if(!pedo_enable_status){
		GSE_LOG(" BMI160 step_counter is not enabled!\n");
		return -1;
	}

	err = BMI160_Get_Pedo_DataReg(priv->client, &pedo_data);
	if(err != 0)
	{
		GSE_LOG(" BMI160_Get_Pedo_DataReg failed!\n");
		return -1;
	}
	//to report the first data
	if(first_data_after_enable){
		GSE_LOG("step counter first_data_after_enable:%d[0x%04x]\n",pedo_data,pedo_data);
		/*if((pedo_data>0)){
			pedo_data --;
		}else{
			pedo_data = 1;
		}*/
		first_data_after_enable = false;
	}
	...
	if(diff>20000){
		...
		if(diff>20000){
			...
			err = BMI160_Reset_Pedo_Data(priv->client,0);
			if(err != 0)
			{
				GSE_LOG(" BMI160_Reset_Pedo_Data failed!\n");
				return -1;
...
static int bmi160_step_c_get_data_step_d(u64 *value, int *status)
{
	return 0;
}
static int bmi160_step_c_get_data_significant(u64 *value, int *status)
{
	return 0;
}
...
static int bmi160_step_c_local_init(void)
{
	...
	if(bmi160_acc_i2c_client == NULL){
		GSE_ERR("bmi160_acc_i2c_client is NULL\n");
		goto bmi160_step_c_creat_attr_failed;
	}

	res = bmi160_step_c_create_attr(&(bmi160_step_c_init_info.platform_diver_addr->driver));
	if(res)
	{
		...
		step_ctl.set_delay = bmi160_step_c_set_delay;
		...
		if (res) {
			...
			goto bmi160_step_c_local_init_failed;
		}
...
bmi160_step_c_local_init_failed:
	bmi160_step_c_delete_attr(&(bmi160_step_c_init_info.platform_diver_addr->driver));
bmi160_step_c_creat_attr_failed:
...
//tad3sgh add --
	...
	register_device_proc("Sensor_accel", "bmi160", "BOSCH");
	GSE_LOG("%s: OK\n", __func__);
	return 0;
...
#ifdef BMI160_STEP_COUNTER /*step counter*/
