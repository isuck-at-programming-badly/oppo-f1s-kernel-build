...
static struct i2c_board_info __initdata i2c_kx023={ I2C_BOARD_INFO(KX023_DEV_NAME, (0x1F))};
...
static int KX023_ReadData(struct i2c_client *client, s16 data[KX023_AXES_NUM])
{
	...
	else
	{
		//eric add for test
		//for(num=0;num<6;num++){
		//GSE_LOG("read sensor data+++++++++++++++buf[num]=%d\n", buf[num]);
		//}
...
static int KX023_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		//GSE_LOG("%s  kx023 %s", buf ,__func__);
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				//printk("%s  ziqing %s", buff ,__func__);
...
#ifdef CONFIG_COMPAT
static long kx023_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
...
static int kx023_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	register_device_proc("Sensor_accel", KX023_DEV_NAME, "KIONIX");
	#endif//VENDOR_EDIT
	GSE_LOG("%s: OK\n", __func__);    
	kx023_init_flag = 0;    
	return 0;
