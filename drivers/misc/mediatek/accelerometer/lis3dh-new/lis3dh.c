...
static int LIS3DH_ReadData(struct i2c_client *client, s16 data[LIS3DH_AXES_NUM])
{
	...
	if((lis_i2c_read_block(client, LIS3DH_REG_OUT_Z+1, &buf[5], 0x01))<0)
	    ...
	    */
	   if((lis_i2c_read_block(client, LIS3DH_REG_OUT_X|0x80, buf, 6))<0)
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    } 
...
static int LIS3DH_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(sensor_suspend == 1)
	{
		...
		//modify by ziqing.guo@BasicDrv.Sensor 2015/07/15 begin
		return -4;
		//end
	}
#if 0
...
static ssize_t show_reg_value(struct device_driver *ddri, char *buf)
{
	...
	for (i = 0x00; i <= 0x3d; i += 0x01){        
	res = lis_i2c_read_block(client, i, &dat,0x01);
	...
	sprintf(_buff,"%s 0x%2x:%2x",_buff,i,dat);
	if(i%0xF == 0)
		strcat(_buff,"\n");
	}
...
static DRIVER_ATTR(reg,      S_IRUGO,  show_reg_value,  NULL);
/*----------------------------------------------------------------------------*/
...
static long lis3dh_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
		...
		switch(cmd)
		{
			...
			/*case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_SENSORDATA, (unsigned long)arg64);
				if(ret < 0)
				{
			...
			case COMPAT_GSENSOR_IOCTL_INIT:
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_INIT, (unsigned long)arg64);
				if(ret < 0)
				{
					printk(KERN_ERR "COMPAT_GSENSOR_IOCTL_INIT is failed!\n");
				}		
			break;
		...
		case COMPAT_GSENSOR_IOCTL_READ_CHIPINFO:
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_CHIPINFO, (unsigned long)arg64);
				if(ret < 0)
				{
		...
		case COMPAT_GSENSOR_IOCTL_READ_GAIN:
			if(arg64 == NULL)
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_GAIN, (unsigned long)arg64);
				if(ret < 0)
				{
		...
		case COMPAT_GSENSOR_IOCTL_READ_OFFSET:
			if(arg64 == NULL)
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_OFFSET, (unsigned long)arg64);
				if(ret < 0)
				{
					printk(KERN_ERR "COMPAT_GSENSOR_IOCTL_READ_OFFSET is failed!\n");
				}
				break;;
		...
		case COMPAT_GSENSOR_IOCTL_READ_RAW_DATA:
			if(arg64 == NULL)
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_RAW_DATA, (unsigned long)arg64);
				if(ret < 0)
				{
		...
		case COMPAT_GSENSOR_IOCTL_SET_CALI:
			if(arg64 == NULL)
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_SET_CALI, (unsigned long)arg64);
				if(ret < 0)
				{
		...
		case COMPAT_GSENSOR_IOCTL_CLR_CALI:
			if(arg64 == NULL)
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_CLR_CALI, (unsigned long)arg64);
				if(ret < 0)
				{
		...
		case COMPAT_GSENSOR_IOCTL_GET_CALI:
			if(arg64 == NULL)
				...
				ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_GET_CALI, (unsigned long)arg64);
				if(ret < 0)
				{
...
static void lis3dh_esd_handle(void)
{
  	 msleep(100);
        printk(KERN_ERR"%s , esd err, reinit device.\n",__func__);
	 lis3dh_enable_nodata(0);
	 LIS3DH_Init(obj_i2c_data->client,1);
	 msleep(100);
	 lis3dh_enable_nodata(1);
}
...
static int lis3dh_get_data(int* x ,int* y,int* z, int* status)
{
	...
	lis_i2c_read_block(obj_i2c_data->client,LIS3DH_REG_CTL_REG1,&data,0x01);
	if (data==0x7)
		lis3dh_esd_handle();

	//modify by ziqing.guo@BasicDrv.Sensor 2015/07/15 begin
	err=LIS3DH_ReadSensorData(obj_i2c_data->client, buff, LIS3DH_BUFSIZE);
	if (err)
		return -1;
	//end
...
static int lis3dh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if ((get_project() == OPPO_15127
	|| get_project() == OPPO_15129
	|| get_project() == OPPO_15130)
	&& get_Operator_Version() == OPERATOR_OPEN_MARKET)
		obj->hw->direction = 4;
	...
	register_device_proc("Sensor_accel", LIS3DH_DEV_NAME, "ST");
	#endif//VENDOR_EDIT
