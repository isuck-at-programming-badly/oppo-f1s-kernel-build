...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
	...
	if (irq == dispsys_irq[DISP_REG_DSI0]) {
		...
		if (reg_val & 0x21f4) { //modify         if (reg_val & 0x1e0)
			static int count = 0; 
		...
		if ((cnt_ovl_underflow[0] >= 150) || (cnt_ovl_underflow[1] >= 150) ||
			(cnt_ovl_underflow[2] >= 150) || (cnt_ovl_underflow[3] >= 150)) {
			BUG();
		}
		#endif
...
#ifdef VENDOR_EDIT
/* pdl@oppo.com,2016/04/28 add for debug system hang */
void test_ovldump(void)
{
	printk("test_ovldump begin\n");
	disp_irq_log_module |= 1;
	if (disp_irq_log_module != 0)
		wake_up_interruptible(&disp_irq_log_wq);
	printk("test_ovldump end\n");
}
#endif
