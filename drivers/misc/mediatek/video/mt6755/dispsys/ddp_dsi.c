...
#ifndef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2016/03/11	Add for BOE's CABC mode MTK patch */
			/* start DSI VM CMDQ */
			if (force_update)
        ...
        if (force_update) {
                  if (cmdq) {
                           DSI_MASKREG32(cmdq, &DSI_REG[d]->DSI_INTSTA, 0x00000020, 0);
                       	   DSI_EnableVM_CMD(module, cmdq);
                           DSI_POLLREG32(cmdq, &DSI_REG[d]->DSI_INTSTA, 0x00000020, 1);
                   } else {
                            wait_vm_cmd_done = false;
                            DSI_EnableVM_CMD(module, cmdq);
                            wait_event_interruptible(_dsi_wait_vm_cmd_done_queue[d], wait_vm_cmd_done);
                   }
             }
 #endif
		} else {
