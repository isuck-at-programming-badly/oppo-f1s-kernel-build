...
#ifdef VENDOR_EDIT
//xiaocheng.li@Swdp.shanghai, 2016/4/10, Add notifier for fb info
...
#ifdef VENDOR_EDIT//Haitao.Zhou@ProDrv,add 2016.01.14 for share backlight level
volatile unsigned int share_backlight_level;
#endif
...
int __parse_tag_videolfb_extra(unsigned long node)
{
	...
	if (size >= sizeof(mtkfb_lcm_name)) {
		DISPCHECK("%s: error to get lcmname size=%lu\n", __func__, size);
		return -1;
	}
