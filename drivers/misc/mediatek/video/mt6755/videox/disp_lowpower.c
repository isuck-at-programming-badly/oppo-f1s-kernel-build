...
#ifdef VENDOR_EDIT
/*liping-m@PhoneSW.Multimedia, 2016/05/13  Add for oled panel MATE mode*/
...
static unsigned int layer_flag=0;
/// static unsigned int last_enter_idle;
msleep(16000);
    while(1){
         msleep(30000);
		...
		// last_enter_idle=enter_idle_time;
         //primary_display
         #if 0
         //read ovl register
         ...
         layer_flag=1;
         #endif
         DISPCHECK("_primary_path_idle_for_meta_mode start");
         ...
         unsigned int ovl0,ovl0_2,ovl1_2;
         ...
         layer_enable[1]=(DISP_REG_GET(DISP_REG_OVL_SRC_CON+ovl_base0_2))&0x03;
         layer_enable[2]=(DISP_REG_GET(DISP_REG_OVL_SRC_CON+ovl_base1_2))&0x03;
		 unsigned int layer_enable_temp;
         layer_enable_temp=DISP_REG_GET(DISP_REG_OVL_SRC_CON+ovl_base0);
         DISP_CPU_REG_SET((DISP_REG_OVL_SRC_CON + ovl_base0),layer_enable_temp&0xfffffff0);
		 layer_enable_temp=DISP_REG_GET(DISP_REG_OVL_SRC_CON+ovl_base0_2);
		 DISP_CPU_REG_SET((DISP_REG_OVL_SRC_CON + ovl_base0_2),layer_enable_temp&0xffffffFC);
		 layer_enable_temp=DISP_REG_GET(DISP_REG_OVL_SRC_CON+ovl_base1_2);
         DISP_CPU_REG_SET((DISP_REG_OVL_SRC_CON + ovl_base1_2),layer_enable_temp&0xffffffFC);
		 {
                 static int count=0;
                 count ++;
                 if(count%2){
        ...
        // _trigger_display_interface(1, NULL, 0);
         primary_display_manual_unlock();
		 DISPCHECK("_primary_path_idle_for_meta_mode end");
         //exit idle
        }
}
#endif
...
void _vdo_mode_enter_idle(void)
{
	int ret = 0;
#ifndef VENDOR_EDIT
//lile@EXP.BasicDrv.LCD, 2016-05-06, add for rm excessive log
	DISPMSG("[disp_lowpower]%s\n", __func__);
#endif
	...
	if (!primary_is_sec()) {
		...
		if (get_lp_cust_mode() > LP_CUST_DISABLE && get_lp_cust_mode() < PERFORMANC_MODE + 1) {
			...
			#ifdef VENDOR_EDIT
				/* liping-m@Mobile Phone Software Dept.Driver, 2016/03/25  Add for lcd frame rate */
				set_fps(50);
			#else
				set_fps(55);
			#endif
...
/*liping-m@PhoneSW.Multimedia, 2016/05/13  Add for oled panel MATE mode*/
#if defined(S6E3FA3_FHD_DSI_CMD)
	 if (disp_helper_get_option(DISP_OPT_IDLE_MGR) &&((get_boot_mode() == NORMAL_BOOT)||(get_boot_mode() == META_BOOT)))
