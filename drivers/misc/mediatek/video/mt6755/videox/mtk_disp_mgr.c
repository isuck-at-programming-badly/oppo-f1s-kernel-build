...
#include "ddp_mmp.h"
//#ifdef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2015/09/28  Add for silence mode */
...
/* liping-m@PhoneSW.Multimedia, 2015/09/28  Add for protect eyes */
	case DISP_IOCTL_SET_GAMMALUT_OFF:
		return "DISP_IOCTL_SET_GAMMALUT_OFF";
	case DISP_IOCTL_GET_GAMMALUT_OFF:
		return "DISP_IOCTL_GET_GAMMALUT_OFF";
	case DISP_IOCTL_SET_GAMMALUT_LEVEL1:
		return "DISP_IOCTL_SET_GAMMALUT_LEVEL1";
	case DISP_IOCTL_GET_GAMMALUT_LEVEL1:
		return "DISP_IOCTL_GET_GAMMALUT_LEVEL1";
    case DISP_IOCTL_SET_GAMMALUT_LEVEL2:
		return "DISP_IOCTL_SET_GAMMALUT_LEVEL2";
    case DISP_IOCTL_GET_GAMMALUT_LEVEL2:
		return "DISP_IOCTL_GET_GAMMALUT_LEVEL2";
    case DISP_IOCTL_SET_GAMMALUT_LEVEL3:
		return "DISP_IOCTL_SET_GAMMALUT_LEVEL3";
    case DISP_IOCTL_GET_GAMMALUT_LEVEL3:
		return "DISP_IOCTL_GET_GAMMALUT_LEVEL3";
//#endif /*VENDOR_EDIT*/
...
/* liping-m@PhoneSW.Multimedia, 2015/09/28  Add for protect eyes */
        case DISP_IOCTL_SET_GAMMALUT_OFF:
            if (cmd == DISP_IOCTL_SET_GAMMALUT_OFF) {
                printk("%s DISP_IOCTL_SET_GAMMALUT_OFF\n", __func__);
            }
        case DISP_IOCTL_GET_GAMMALUT_OFF:
            if (cmd == DISP_IOCTL_GET_GAMMALUT_OFF) {
                printk("%s DISP_IOCTL_GET_GAMMALUT_OFF\n", __func__);
            }
        case DISP_IOCTL_SET_GAMMALUT_LEVEL1:
            if (cmd == DISP_IOCTL_SET_GAMMALUT_LEVEL1) {
                printk("%s DISP_IOCTL_SET_GAMMALUT_LEVEL1\n", __func__);
            }
        case DISP_IOCTL_GET_GAMMALUT_LEVEL1:
            if (cmd == DISP_IOCTL_GET_GAMMALUT_LEVEL1) {
                printk("%s DISP_IOCTL_GET_GAMMALUT_LEVEL1\n", __func__);
            }
        case DISP_IOCTL_SET_GAMMALUT_LEVEL2:
            if (cmd == DISP_IOCTL_SET_GAMMALUT_LEVEL2) {
                printk("%s DISP_IOCTL_SET_GAMMALUT_LEVEL2\n", __func__);
            }
        case DISP_IOCTL_GET_GAMMALUT_LEVEL2:
            if (cmd == DISP_IOCTL_GET_GAMMALUT_LEVEL2) {
                printk("%s DISP_IOCTL_GET_GAMMALUT_LEVEL2\n", __func__);
            }
        case DISP_IOCTL_SET_GAMMALUT_LEVEL3:
            if (cmd == DISP_IOCTL_SET_GAMMALUT_LEVEL3) {
                printk("%s DISP_IOCTL_SET_GAMMALUT_LEVEL3\n", __func__);
            }
        case DISP_IOCTL_GET_GAMMALUT_LEVEL3:
            if (cmd == DISP_IOCTL_GET_GAMMALUT_LEVEL3) {
...
#ifdef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2015/11/05  Add for HBM mode */
...
extern int primary_display_set_jdi_reg(const char *buf);

extern int primary_display_read_jdi_reg(const char *buf);
...
static ssize_t sysfs_store_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    printk("%s JDI_SET_REG =%s\n", __func__, regbuf);
		...
		const char *buf, size_t count)
{
	...
	int ret = 0;
	regbuf = buf;
	ret = primary_display_set_jdi_reg(buf);
	return count;
}

static DEVICE_ATTR(read, (S_IRUGO | S_IWUGO),sysfs_show_read, sysfs_store_read);
static DEVICE_ATTR(write, 0644,sysfs_store_show, sysfs_store_write);
#endif
#endif /*VENDOR_EDIT*/
#ifdef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2015/09/28  Add for silence mode */
...
#ifdef VENDOR_EDIT
/* liping-m@PhoneSW.Multimedia, 2016/02/29  Add for LCD cabc mode */
#if defined(ILI9881C_FHD720_DSI_VDO_BOE) || defined(OPPO16021_HX8394_TRULY_HD720_DSI_VDO)
...
unsigned long CABC_mode = 3;
#endif

#if defined(ILI9881C_FHD720_DSI_VDO_BOE) || defined(OPPO_HX8394_TRULY_HD720_DSI_VDO)  || defined(OPPO16021_HX8394_TRULY_HD720_DSI_VDO) || defined(OPPO_JDI_R63452_FHD1080_DSI_CMD)
...
#ifdef VENDOR_EDIT
/* Xinqin.Yang@PhoneSW.Multimedia, 2015/05/26  Add for HBM mode */
	dev =(struct device *) class_dev;
#if defined(S6E3FA3_FHD_DSI_CMD)
    ret = device_create_file(dev, &dev_attr_LCM_HBM);
    if (ret < 0)
    {
        printk("%s YXQ device create file failed!\n", __func__);
    }

	ret = device_create_file(dev, &dev_attr_read);
    if (ret < 0)
    {
        printk("%s liping-m device create file failed!\n", __func__);
    }

	ret = device_create_file(dev, &dev_attr_write);
    if (ret < 0)
    {
        printk("%s liping-m device create file failed!\n", __func__);
    }
...
/* Xinqin.Yang@PhoneSW.Multimedia, 2015/06/05  Add for silence mode */
    if (oppo_boot_mode == OPPO_SILENCE_BOOT) {
        printk("%s oppo_boot_mode is OPPO_SILENCE_BOOT, set silence_mode to 1\n", __func__);
        silence_mode = 1;
   }
...
#ifdef VENDOR_EDIT
	/* liping-m@PhoneSW.Multimedia, 2016/02/29  Add for CABC mode */
