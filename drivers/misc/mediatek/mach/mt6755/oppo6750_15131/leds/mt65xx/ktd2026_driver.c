...
#define LED_OUT_ALON  (LED2_OUT_ALON)|(LED3_OUT_ALON)
#define LED_OUT_PWM1 (LED2_OUT_PWM1)

static struct i2c_board_info __initdata Breathlight_i2c_info ={ I2C_BOARD_INFO(KTD_I2C_NAME, 0x30)};
...
/*
	...
	*/
	...
	if(value == 0) {
		...
		i2c_smbus_write_byte_data(ktd20xx_client, 0x00, 0x20);// initialization LED off
		i2c_smbus_write_byte_data(ktd20xx_client, 0x06, 0x00);//set current is D1mA
		i2c_smbus_write_byte_data(ktd20xx_client, 0x07, 0x1D);//set current is D2mA
		i2c_smbus_write_byte_data(ktd20xx_client, 0x08, 0x03);//set current is D3mA
		i2c_smbus_write_byte_data(ktd20xx_client, 0x04, LED_OUT_ALON);//turn om all of leds
	}	
}
...
static int ktd20xx_probe(struct i2c_client *client, const struct i2c_device_id *id){
	...
	*/
	if (led_classdev_register(&client->dev, &KTD2026_lcds[0])) {
            printk(KERN_ERR "led_classdev_register failed of KTD2026_lcds!\n");    
    }
	...
	ret = i2c_smbus_write_byte_data(ktd20xx_client, 0x00, 0x07);//reset reg
...
err_group_register:
	led_classdev_unregister(&KTD2026_lcds[0]);	
err_i2c_check:	
	ktd20xx_client = NULL;
	return ret;
}
