...
static int32_t cmdq_core_task_realloc_buffer_size(TaskStruct *pTask, uint32_t size)
{
	...
	pTask->useEmergencyBuf = useEmergencyBuf;
...
static int32_t cmdq_core_check_task_valid(TaskStruct *pTask)
{
	...
	for (va = (uint64_t *)pTask->pVABase; ret &&
		(unsigned long)(va + 1) <
		(unsigned long)pTask->pVABase + pTask->commandSize; va++)
...
#ifdef VENDOR_EDIT
//jie.cheng@swdp.sh,2016/03/31, do not core dump to prevent the kernel panic and add the null pointer check for pDumpTask->pCMDEnd
		    if (pDumpTask->pCMDEnd != NULL)
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
//jie.cheng@swdp.sh,2016/03/31, do not core dump to prevent the kernel panic
		...
		while (0 == waitQ || CMDQ_INVALID_THREAD == pTask->thread) {
			continue_wait = false;
			mutex_lock(&gCmdqTaskMutex);
			...
			if (CMDQ_INVALID_THREAD == pTask->thread) {
				...
				cmdq_core_dump_task(pTask);
				cmdq_core_dump_task_with_engine_flag(pTask->engineFlag);
				CMDQ_AEE("CMDQ", "task 0x%p timeout with invalid thread\n", pTask);
				continue_wait = true;
			}
