...
static int lsm6ds3_do_cali(struct i2c_client *client, int *cali_sw){
	...
	if(res<0){
		...
		if(((x-x_max)<-50)||((x-x_max)>50)||((y-y_max)<-50)||((y-y_max)>50)||((z-z_max)<-50)||((z-z_max)>50)){
			cali_res = -100;
			GYRO_ERR("max cali data %d,%d,%d avg %d,%d,%d\n",x_max,y_max,z_max,x,y,z);
			GYRO_ERR("data vary fail!!\n");
			return -1;
		}
		cali_sw[LSM6DS3_AXIS_X]=cali_data[0];
		cali_sw[LSM6DS3_AXIS_Y]=cali_data[1];
		cali_sw[LSM6DS3_AXIS_Z]=cali_data[2];
		cali_res = 0;
