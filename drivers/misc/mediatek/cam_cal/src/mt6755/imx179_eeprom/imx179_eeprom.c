...
static char AF[4]={0};
static char S5K3H7_AF[5]={0};

extern int get_otp_af(char * af);
extern int s5k3h7_get_otp_af(char * af);


/*******************************************************************************
*
********************************************************************************/
static struct i2c_board_info kd_cam_cal_dev __initdata = { I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0xA0 >> 1)};
...
static bool byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALERR("[CAM_CAL] fail imx179_byteread_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
		return false;
	}
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		/*if(selective_read_byte(addr,(u8*)buff,imx179_DEVICE_ID)){*/
...
static int iReadCAM_CAL_8(u16 a_u2Addr, u8 * a_puBuff, u16 i2c_id)
 {
     int  i4RetValue = 0;
     ...
     CAM_CALDB("oppo  [CAM_CAL][iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
...
static void IMX179_ReadOtp(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
    kal_uint16 i = 0;
    u8 readbuff ;
    int ret ;
    u16 i2c_id = 0x20/*IMX179_DEVICE_ID*/;
    CAM_CALDB("[CAM_CAL]ENTER  address:0x%x buffersize:%d\n ", address, buffersize);
    
    
    for(i = 0; i<buffersize; i++)
    {               
        ret= iReadCAM_CAL_8(address+i,&readbuff, i2c_id);
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
    int  i4RetValue = 0;
     ...
     /* read otp */
    //IMX179_ReadOtp(ui4_offset,pinputdata, ui4_length);
    if(ui4_offset == 0x3424){//read imx179 af macro
        *pinputdata = AF[0];
        *(pinputdata+1) = AF[1];
        }
    else if(ui4_offset == 0x3426){//read imx179 af infi
        *pinputdata = AF[2];
        *(pinputdata+1) = AF[3];
    }
    else if(ui4_offset == 0x0A23){//read s5k3h7 af macro
        *pinputdata = S5K3H7_AF[0];
        *(pinputdata+1) = S5K3H7_AF[1];
    }
    else if(ui4_offset == 0x0A25){//read s5k3h7 af infi
        *pinputdata = S5K3H7_AF[2];
        *(pinputdata+1) = S5K3H7_AF[3];
    }
    else if(ui4_offset == 0x0A08){//read s5k3h7 id
        *pinputdata = S5K3H7_AF[4];
        CAM_CALDB( "[[CAM_CAL]]vendor id=%d\n",*pinputdata);
    }
    ...
    CAM_CALDB("[IMX179EEPORM] iReadData done\n" );
   return ui4_length;
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = IMX179_DEVICE_ID >> 1;
	spin_unlock(&g_CAM_CALLock); /* for SMP*/
...
{
	CAM_CALDB("[CAM_CAL] IMX179 CAM_CAL_i2C_init Start!\n");
