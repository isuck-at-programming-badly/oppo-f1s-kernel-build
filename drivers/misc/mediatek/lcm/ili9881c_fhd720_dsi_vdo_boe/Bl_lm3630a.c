...
/* liping-m@PhoneSW.Multimedia, 2016/02/26	ADD for lcm cabc*/
bool pwm_set_flag = false;
bool cabc_set_flag = false;

extern unsigned int oppo_lcm_is_which;
extern int lcm_set_CABC_mode(int mode);
extern int lcm_set_CABC_mode_truly(int mode);
extern int lcm_set_CABC_mode_tianma(int mode);
#endif
...
void lcm_setbacklight_pwm(int mode)
{
       if((mode)&& (!pwm_set_flag)){
               LM3630A_Write_Byte(0x01,0x19);//open PWM
               pwm_set_flag =true;
       }else if(pwm_set_flag){
...
int lm3630a_setbacklight(unsigned int level)
{
 ...
 if(oppo_lcm_is_which==1){
  if((level < 20)&&(2 <= level))
	{
		printk("%s BOE LCD close CABC \n", __func__);
	      LM3630A_Write_Byte(0x01,0x18); 
		  cabc_set_flag=true;
		  level = 2+(level-1)*18/20;
		  if(level < 2)
				level = 2;
    }else if((level > 20)&&( cabc_set_flag==true)){
		printk("%s BOE LCD open CABC \n", __func__);
		  LM3630A_Write_Byte(0x01,0x19); 
    	  cabc_set_flag=false;
    }
 }else if(oppo_lcm_is_which==2){
	 if((level < 20)&& (2 <= level))
	   {
		printk("%s TRULY LCD close CABC \n", __func__);		   
	      LM3630A_Write_Byte(0x01,0x18); 
		  cabc_set_flag=true;
		  level = 2+(level-1)*18/20;
		  if(level < 2)
	 ...
	 if((level < 20)&& (2 <= level))
	   {
			printk("%s TIANMA LCD close CABC \n", __func__);	
	      	LM3630A_Write_Byte(0x01,0x18); 
		 	 cabc_set_flag=true;
		  	level = 2+(level-1)*18/20;
		  	if(level < 2)
			...
			printk("%s TIANMA LCD open CABC \n", __func__);	
			LM3630A_Write_Byte(0x01,0x19); 
			cabc_set_flag=false;
	   }
 }
#endif  /**ILI9881C_FHD720_DSI_VDO_BOE**/


    if (level == 0) {
		LM3630A_Write_Byte(0x00,0x9F);
