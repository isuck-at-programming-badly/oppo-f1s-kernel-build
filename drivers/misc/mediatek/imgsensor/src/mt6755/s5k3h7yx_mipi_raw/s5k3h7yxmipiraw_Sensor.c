...
static int s5k3h7_update_otp_wb(void);
static int s5k3h7_update_awb_gain(uint32_t R_GAIN,uint32_t G_GAIN,uint32_t B_GAIN);
static int read_otp(uint32_t page, uint32_t address, uint32_t* array, uint32_t size);
//static uint32_t read_otp_wb_s5k3h7(uint32_t index, struct otp_struct* otp);
static int check_otp_awb(uint32_t index);
static int s5k3h7_read_otp_af(void);
...
#define RG_TYPICAL_SUNNY     0x112  
#define BG_TYPICAL_SUNNY     0x135 

#define RG_TYPICAL_TRULY     0x113	 
#define BG_TYPICAL_TRULY    0x123 

static char vendor_id = 0;
...
static void write_shutter(kal_uint32 shutter)
{
    ...
    kal_uint32 defult_line_length_pix = 3668;
...
#ifdef VENDOR_EDIT
//jindian.guan@camera driver 20160223 modify for slow shutter
        if(longshot_state){
            LOG_INF("1! longshot state\n");
            write_cmos_sensor(0x0342, line_length_pix);
		    write_cmos_sensor(0x0340, max_frame_length);
            write_cmos_sensor(0x0202, max_exposure_length);
        }else{
            // Update Shutter
		    //write_cmos_sensor_8(0x0104,0x01);
		    write_cmos_sensor(0x0340, imgsensor.frame_length);
...
//jindian.guan add for otp
    ...
    if (1==s5k3h7_read_otp_af()){
              LOG_INF("s5k3h7 AF otp data error!!\n");
    }

    if (1==s5k3h7_update_otp_wb()) {
        //zl:OTP data error:module ID error,or otp data empty
        LOG_INF("s5k3h7 wb otp data error!!\n");
...
int s5k3h7_get_otp_af(char * af)
{
   ...
   AF[4]=(u8)vendor_id;
   LOG_INF("s5k3h7 af data:inf_h=0x%x, inf_l=0x%x,macro_h=0x%x,macro_l=0x%x,vendor_id=0x%x\n",AF[3],AF[2],AF[1],AF[0],AF[4]);
   memcpy(af,AF,5);
   return ERROR_NONE;
}
static int s5k3h7_read_otp_af(void)
{
    ...
    for (i=1;i<4;i++) {
        ...
        write_cmos_sensor_8(0x0100, 0x01); // Set ECC
        mdelay(5);
        write_cmos_sensor(0xFCFC, 0xD000); // read ON

        write_cmos_sensor_8(0x0A02, i); //Select Page
        write_cmos_sensor_8(0x0A00, 0x01);
        write_cmos_sensor_8(0x0A01, 0x01);
        mdelay(5);
        temp = read_cmos_sensor_8(0x0A21);
        LOG_INF(" check_otp_af page %d result 0x%x\n",i ,temp);
        if (temp == 0x40) {
            //valid data
            AF[1]=(u8)(read_cmos_sensor_8(0x0A23));
            AF[0]=(u8)(read_cmos_sensor_8(0x0A24));
            AF[3]=(u8)(read_cmos_sensor_8(0x0A25));
            AF[2]=(u8)(read_cmos_sensor_8(0x0A26));
            break;
        }
    }
    AF[4]=(u8)vendor_id;
...
static int read_otp(uint32_t page, uint32_t address, uint32_t* array, uint32_t size)
{
    ...
    write_cmos_sensor_8(0x0A02, page); //Select Page
    write_cmos_sensor_8(0x0A00, 0x01);
    write_cmos_sensor_8(0x0A01, 0x01);
    mdelay(5);
    for (i=0;i<size;i++) {
        *(array+i)=read_cmos_sensor_8(address+i);
            LOG_INF(" s5k3h7_otp read_from_otp [%d] == 0x%x\n",i,array[i]);
    }
    LOG_INF(" s5k3h7_otp page = 0x%x\n",page);
...
static int check_otp_awb(uint32_t index)        //zl:check flag
{
    ...
    id = read_cmos_sensor_8(0x0A08);
    

    LOG_INF("s5k3h7_otp check_otp_flag:reg 0x0A04 page=%d  flag=%x  vendorid=%x\n",page,flag,id);
...
static uint32_t read_otp_wb_s5k3h7(uint32_t index, struct otp_struct* otp)
{
    ...
    start_address=0x0A05;
    temp=read_otp(index+1, start_address, arr, 28);

    if (temp==1) {
        otp->customer_id = arr[3];
        vendor_id = otp->customer_id;
        otp->year = arr[0];
        otp->month = arr[1];
        otp->day = arr[2];
        ...
        current_r = ((arr[20]&0x03)<<8)+arr[21];
        current_b = ((arr[20]&0x0C)<<6)+arr[22];
        current_gr = ((arr[20]&0x30)<<4)+arr[23];
        current_gb = ((arr[20]&0xC0)<<4)+arr[24];
        current_g  = (current_gr + current_gb) / 2;
        otp->rg_ratio =512 * current_r /current_g;
        otp->bg_ratio =512 * current_b /current_g;


    }
    else {
        LOG_INF(" s5k3h7_otp read all otp fail\n");
        return 0;
    }
    return 1;
}

static int s5k3h7_update_awb_gain(uint32_t R_GAIN,uint32_t G_GAIN,uint32_t B_GAIN)//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{

        LOG_INF("s5k3h7_otp R_GAIN=0x%x G_GAIN=0x%x, B_GAIN=0x%x\n", R_GAIN, G_GAIN, B_GAIN);
        ...
        write_cmos_sensor_8(0x0214, G_GAIN>>8);    //zl:why rewrite?
        write_cmos_sensor_8(0x0215, G_GAIN& 0xFF);
        write_cmos_sensor(0x0B00, 0x0132);
        LOG_INF("s5k3h7_otp ok\n");
        return 0;
}
...
static int s5k3h7_update_otp_wb(void)
{
    ...
    LOG_INF(" s5k3h7_otp update_otp_wb++\n");
    for (i=0;i<3;i++) {
        temp = check_otp_awb(i);
        LOG_INF(" check_otp_awb page %d result %d\n",i ,temp);
        if (temp == 2) {
            //valid data
            break;
        }
    ...
    if (i==3){
        ...
        LOG_INF("s5k3h7_otp  no valid wb OTP data\n");
        return 1;
       }
    memset(&current_otp, 0, sizeof(current_otp));
    wb_flag = read_otp_wb_s5k3h7(otp_index, &current_otp);
    if (true == wb_flag){
        //2016/04/15 lizhiqian modify for distinguish diffrent module QTECH or SUNNY
        LOG_INF(" s5k3h7_otpcurrent_otp->customer_id = 0x%x\n",current_otp.customer_id);
        if (current_otp.customer_id == 0x02) {
            RG_Ratio_typical = RG_TYPICAL_TRULY;
            BG_Ratio_typical = BG_TYPICAL_TRULY;
            //GG_Ratio_typical = GG_Ratio_typical_QTECH;
        }
        else if (current_otp.customer_id == 0x01){
            RG_Ratio_typical = RG_TYPICAL_SUNNY;
            BG_Ratio_typical = BG_TYPICAL_SUNNY;
        }
        else {
            LOG_INF(" s5k3h7_otp it is not QTECH or SUNNY module = 0x%x\n",current_otp.customer_id);
            return 1;
        }
        //calculate gain
        //0x400 = 1x gain
        LOG_INF(" s5k3h7_otp current_otp.rg_ratio = 0x%x\n",current_otp.rg_ratio);
        LOG_INF(" s5k3h7_otp current_otp.bg_ratio = 0x%x\n",current_otp.bg_ratio);
        LOG_INF(" s5k3h7_otp RG_Ratio_typical = 0x%x\n",RG_Ratio_typical);
        LOG_INF(" s5k3h7_otp BG_Ratio_typical = 0x%x\n",BG_Ratio_typical);
        if (current_otp.rg_ratio==0||current_otp.bg_ratio==0){
            LOG_INF("s5k3h7_otp failed,current_otp.rg_ratio==0 ||current_otp.bg_ratio==0\n ");
            return 1;
        }
#define standard_value  256
        ...
        if (r_ratio==0||b_ratio==0){
            LOG_INF("s5k3h7_otp failed,r_ratio==0 ||b_ratio==0\n ");
            return 1;
        }
        ...
        s5k3h7_update_awb_gain(R_GAIN,G_GAIN,B_GAIN);
    }
    else {
        LOG_INF("s5k3h7_otp read faile %d\n",__LINE__);
        return 1;
    }
