...
#define DEVICE_VERSION_OV5695		"ov5695"
#define DEVICE_MANUFACUTRE_OV5695	"Omnivision"
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap = {
		...
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 2560,//jindian.guan modify for cts 2592
		.grabwindow_height = 1920,//jindian.guan modify for cts 1944
...
#ifndef VENDOR_EDIT
/*zhaozhengtao modify for video and non-zsd preview*/
	.normal_video = {
		.pclk = 45000000,
		.linelength = 672,
		.framelength = 2232,
...
/*oppo hufeng 20150515 add for device list*/
            			if(deviceInfo_register_value==0x00){
                			register_device_proc("Cam_f", DEVICE_VERSION_OV5695, DEVICE_MANUFACUTRE_OV5695);
...
(*otp_ptr).sensor_id = read_cmos_sensor( addr + 2);
temp = read_cmos_sensor( addr + 5);
(*otp_ptr).rg_ratio = (read_cmos_sensor(addr+3)<<2) + ((temp>>6) & 0x03);
(*otp_ptr).bg_ratio = (read_cmos_sensor( addr + 4)<<2) + ((temp>>4) & 0x03);
}
else
{
...
LOG_INF("Read module_integrator_id 0x%x, lens_id: 0x%x, rg_ratio: 0x%x, bg_ratio: 0x%x\n", 
...
{
int RG_Ratio_Typical = 0xFD, BG_Ratio_Typical = 0x135;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
#ifndef VENDOR_EDIT
/*zhaozhengtao modify for video and non-zsd preview 2016-03-08*/
