...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap = {//30fps 74.4M bps/lane
		.pclk = 144000000,//this value just for calculate shutter
...
static void sensor_init(void)
{
	...
	//for auto lens shading nick 0510
	write_cmos_sensor(0x583e,0x20);// 16x
	write_cmos_sensor(0x583f,0x10);// ;8x
	write_cmos_sensor(0x5840,0x18);// min_Q
	write_cmos_sensor(0x5841,0x0d);// ; enable
...
/*oppo hufeng 20150515 add for device list*/
            			if(deviceInfo_register_value==0x00){
                			register_device_proc("Cam_f", DEVICE_VERSION_OV8865, DEVICE_MANUFACUTRE_OV8865);
...
//jindian.guan add for otp
	...
	if(otp_update()==1)
	{
	    LOG_INF("OTP Apply Successed\n");
	}
	else
	{
	    LOG_INF("OTP Apply Failed\n");
	}
#endif
	spin_lock(&imgsensor_drv_lock);
