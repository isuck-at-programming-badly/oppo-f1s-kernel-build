...
//xiongxing 2016-05-03, add for distinguish  R9 mode from F1 
#define VCM_R9 (0x53)
#define VCM_F1 (0x14)
#endif
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			...
			#ifdef VENDOR_EDIT_15311
            selective_read_eeprom_3l8(0x0A, data_VCM); //xiongxing 2016-05-03, add for distinguish  R9 mode from F1  
            if ((*sensor_id == imgsensor_info.sensor_id) && (data_VCM[0] == VCM_R9)) {  
			#else
...
/*oppo hufeng 20150515 add for device list*/
            if(deviceInfo_register_value==0x00){
                register_device_proc("Cam_b", DEVICE_VERSION_S5K3L8, DEVICE_MANUFACUTRE_S5K3L8);
                deviceInfo_register_value=0x01;
            }
#endif
                printk("i2c write id: 0x%x, sensor id: 0x%x module_id0x%x\n", imgsensor.i2c_write_id,*sensor_id,data[0]);
                return ERROR_NONE;
            }   
