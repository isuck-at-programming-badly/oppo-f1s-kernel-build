...
#define DEVICE_VERSION_OV5648		"ov5648"
#define DEVICE_MANUFACUTRE_OV5648	"Omnivision"
static uint8_t deviceInfo_register_value = 0x00; 
#endif
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
#define OV5648OTP /*jindian.guan add for OTP*/
...
#define RG_Ratio_Typical_old   0x16b
#define BG_Ratio_Typical_old   0x18d

#define RG_Ratio_Typical_new   0x138
#define BG_Ratio_Typical_new   0x159
...
int check_otp_ov5648(int index)
{
    int flag, i;
    int rg, bg;
    if (index == 1) {
        ...
        printk(" ov5648 flag == 0x%x,index == 0x%x,rg == 0x%x,bg == 0x%x\n",flag,index,rg,rg);
        ...
        LOG_INF(" ov5648 flag == 0x%x,index == 0x%x,rg == 0x%x,bg == 0x%x\n",flag,index,rg,rg);
    }
    else if (index == 3) {
        // read otp --Bank 1
        write_cmos_sensor(0x3d84, 0xc0);
        ...
        write_cmos_sensor(0x3d81, 0x01);
        mDELAY(5);
        flag = read_cmos_sensor(0x3d07);
        rg = read_cmos_sensor(0x3d09);
        bg = read_cmos_sensor(0x3d0a);
    }
    flag =flag &0x80;
    // clear otp buffer
    for (i=0;i<16;i++)
...
int ov5648_read_otp(int index, struct otp_struct *otp_ptr)
{
    ...
    if (index == 1) {
        ...
        (*otp_ptr).rg_ratio = read_cmos_sensor(0x3d07);
        (*otp_ptr).bg_ratio = read_cmos_sensor(0x3d08);
        (*otp_ptr).rg_ratio = ((*otp_ptr).rg_ratio<<2) + ((temp>>6) & 0x03);
        (*otp_ptr).bg_ratio = ((*otp_ptr).bg_ratio<<2)+ ((temp>>4) & 0x03);
        //a1=read_cmos_sensor(0x3d07);
        //a2=read_cmos_sensor(0x3d08);
        //a3=read_cmos_sensor(0x3d0b);

        //LOG_INF(" ov5648 a1 == 0x%x,a2 == 0x%x,a3== 0x%x\n",a1,a2,a3);
        (*otp_ptr).light_rg = 0;//(read_cmos_sensor(0x3d0c)<<2) + (temp>>2) & 0x03;
        (*otp_ptr).light_bg = 0;//(read_cmos_sensor(0x3d0d)<<2) + temp & 0x03;
        (*otp_ptr).user_data[0] = read_cmos_sensor(0x3d09);
        (*otp_ptr).user_data[1] = read_cmos_sensor(0x3d0a);
        LOG_INF(" ov5648 module_id == 0x%x,lens_id == 0x%x,rg == 0x%x,bg == 0x%x\n",(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,(*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio);
    }
        ...
        (*otp_ptr).rg_ratio = read_cmos_sensor(0x3d00);
        (*otp_ptr).bg_ratio = read_cmos_sensor(0x3d01);
        (*otp_ptr).rg_ratio = ((*otp_ptr).rg_ratio<<2) + ((temp>>6) & 0x03);
        (*otp_ptr).bg_ratio = ((*otp_ptr).bg_ratio<<2)+ ((temp>>4) & 0x03);
        (*otp_ptr).light_rg = 0;//(read_cmos_sensor(0x3d05)<<2) + (temp>>2) & 0x03;
        (*otp_ptr).light_bg = 0;//(read_cmos_sensor(0x3d06)<<2) + temp & 0x03;
        ...
        write_cmos_sensor(0x3d81, 0x01);
        mDELAY(5);
        (*otp_ptr).module_integrator_id = (read_cmos_sensor(0x3d07) & 0x7f);
        (*otp_ptr).lens_id = read_cmos_sensor(0x3d08);
        temp = read_cmos_sensor(0x3d0d);
        (*otp_ptr).rg_ratio = read_cmos_sensor(0x3d09);
        (*otp_ptr).bg_ratio = read_cmos_sensor(0x3d0a);
        (*otp_ptr).rg_ratio = ((*otp_ptr).rg_ratio<<2) + ((temp>>6) & 0x03);
        (*otp_ptr).bg_ratio = ((*otp_ptr).bg_ratio<<2)+ ((temp>>4) & 0x03);
        (*otp_ptr).light_rg = 0;//(read_cmos_sensor(0x3d0e)<<2) + ((temp>>2) & 0x03);
        (*otp_ptr).light_bg = 0;//(read_cmos_sensor(0x3d0f)<<2) + (temp & 0x03);
        (*otp_ptr).user_data[0] = read_cmos_sensor(0x3d0b);
        (*otp_ptr).user_data[1] = read_cmos_sensor(0x3d0c);
    }
    // clear otp buffer
    for (i=0;i<16;i++) {
            write_cmos_sensor(0x3d00 + i, 0x00);
...
int ov5648_update_otp()
{
    ...
    int rg,bg;
    int RG_Ratio_Typical=0x135,BG_Ratio_Typical=0x144;
    // R/G and B/G of current camera module is read out from sensor OTP
    // check first OTP with valid data
    for (i=1;i<=3;i++) {
        temp = check_otp_ov5648(i);
        if (temp == 2){
            otp_index = i;
            LOG_INF("ov5648 otp data i=0x%x\n",i);
            break;
        }
    }
    if (i>3) {
        // no valid wb OTP data
        LOG_INF(" ov5648 no valid wb OTP data\n");
        return 1;
    }
    ...
    LOG_INF(" ov5648 current_otp.rg_ratio=0x%x,current_otp.bg_ratio=0x%x\n",rg,bg);
    if (current_otp.lens_id==0x41){
        LOG_INF("OV5648 new module\n");
        RG_Ratio_Typical=RG_Ratio_Typical_new;
        BG_Ratio_Typical=BG_Ratio_Typical_new;
    }
    else if (current_otp.lens_id==0x01){
        LOG_INF("OV5648 old module\n");
        RG_Ratio_Typical=RG_Ratio_Typical_old;
        BG_Ratio_Typical=BG_Ratio_Typical_old;
    }
    else {
        LOG_INF("OV5648 module,not do otp correction\n");
        return 0;
    }
    ...
    printk(" ov5648 R_gain == 0x%x\n",R_gain);
    printk(" ov5648 G_gain == 0x%x\n",G_gain);
    printk(" ov5648 B_gain == 0x%x\n",B_gain);
...
#ifdef OV5648OTP
/*jindian.guan add for OTP*/
    kal_uint16 sta_otp_flag = 0;
    LOG_INF("ov5648 before ov5648_ov5648_update_otp_wb :\n ");
    sta_otp_flag = ov5648_update_otp();
    if (1==sta_otp_flag){
        //zl:OTP data error:module ID error,or otp data empty
        LOG_INF("ov5648 otp data error!!\n");
    }
#endif
...
/*oppo hufeng 20150515 add for device list*/
			if(deviceInfo_register_value==0x00){
				register_device_proc("Cam_f", DEVICE_VERSION_OV5648, DEVICE_MANUFACUTRE_OV5648);
