...
#ifdef VENDOR_EDIT
//jindian.guan add for otp
#define USHORT             unsigned short

static int imx179_update_otp_wb(void);
static int imx179_update_awb_gain(uint32_t R_GAIN,uint32_t G_GAIN,uint32_t B_GAIN);
static int read_otp(uint32_t page, uint32_t address, uint32_t* array, uint32_t size);
//static uint32_t read_otp_wb_imx179(uint32_t index, struct otp_struct* otp);
static int check_otp_awb(uint32_t index);
...
#define RGr_Ratio_Typical_QTECH   0x1FE        
#define BGr_Ratio_Typical_QTECH   0x241
#define RGr_Ratio_Typical_SUNNY   0x1DE
#define BGr_Ratio_Typical_SUNNY   0x232
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		.pclk = 224000000,//256000000				//record different mode's pclk
		.linelength = 3440,				//record different mode's linelength
		.framelength = 2171,//2481			//record different mode's framelength
...
//jindian.guan modify
	...
	.normal_video = {
		...
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 1940,
		.grabwindow_height = 1092,
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
	},
#endif
...
static void set_shutter(kal_uint32 shutter)
{
    ...
    #ifdef VENDOR_EDIT
    //jindian.guan@camera driver 20160223 add for slow shutter
    kal_bool   longshot_state = KAL_FALSE;
    kal_uint32 defult_line_length_pix = 3440;
    kal_uint32 max_frame_length = 0xFFFF;
    kal_uint32 max_exposure_length = 0xFFEF;
    kal_uint32 line_length_pix = shutter;
    if(shutter > 50000){
        longshot_state = KAL_TRUE;
        line_length_pix = (shutter*defult_line_length_pix)/max_exposure_length;
     }
    LOG_INF("Exit! shutter =%d,longshot_state =%d, line_length_pix =%d\n",shutter, longshot_state,line_length_pix);
    #endif
    spin_lock(&imgsensor_drv_lock);
    ...
    if (imgsensor.autoflicker_en) {
        ...
        // Extend frame length
        if(longshot_state){
            LOG_INF("1! longshot state\n");
            write_cmos_sensor_8(0x0104, 0x01);
            write_cmos_sensor(0x0342, (line_length_pix >> 8) & 0xFF);
		    write_cmos_sensor(0x0343, line_length_pix & 0xFF);
		    write_cmos_sensor(0x0340, (max_frame_length >> 8) & 0xFF);
		    write_cmos_sensor(0x0341, max_frame_length & 0xFF);
		    write_cmos_sensor_8(0x0104, 0x00); 
    ...
    // Update Shutter
    if(longshot_state){
        LOG_INF("2! longshot state\n");
        write_cmos_sensor_8(0x0104, 0x01);
	    write_cmos_sensor(0x0202, (max_exposure_length >> 8) & 0xFF);
	    write_cmos_sensor(0x0203, max_exposure_length  & 0xFF);
	    write_cmos_sensor_8(0x0104, 0x00);
    }else{
	//    write_cmos_sensor_8(0x0104, 0x01);
...
//jindian.guan add for otp
    if (1==read_otp_af()){
            LOG_INF("imx179 AF otp data error!!\n");
     }

    if (1==imx179_update_otp_wb()) {
        //zl:OTP data error:module ID error,or otp data empty
        LOG_INF("imx179 wb otp data error!!\n");
        //return ERROR_SENSOR_CONNECT_FAIL;//ERROR_NONE;
    }
#endif
}	/*	sensor_init  */
...
{
	//1640x1232   PO:560MHZ   pclk:224   LL:3440  FL:2171
	LOG_INF("E\n");
      write_cmos_sensor(0x41C0, 0x01);
         ...
         write_cmos_sensor(0x030B, 0x01);
         write_cmos_sensor(0x030C, 0x00);
         write_cmos_sensor(0x030D, 0x8C);
         write_cmos_sensor(0x0340, 0x08);
         write_cmos_sensor(0x0341, 0x7B);
         write_cmos_sensor(0x0342, 0x0D);
...
//		set_mirror_flip(IMAGE_HV_MIRROR);
		...
		write_cmos_sensor(0x034B,0x6B);
		write_cmos_sensor(0x034C,0x07);
		write_cmos_sensor(0x034D,0x94);
		write_cmos_sensor(0x034E,0x04);
		write_cmos_sensor(0x034F,0x44);
		write_cmos_sensor(0x0383,0x01);
		write_cmos_sensor(0x0387,0x01);
		write_cmos_sensor(0x0390,0x00);
		...
		write_cmos_sensor(0x33D4,0x0C);
		write_cmos_sensor(0x33D5,0xD0);
		write_cmos_sensor(0x33D6,0x07);
		write_cmos_sensor(0x33D7,0x36);
		write_cmos_sensor(0x4100,0x0E);
		write_cmos_sensor(0x4108,0x01);
		write_cmos_sensor(0x4109,0x7C);
		write_cmos_sensor(0x3302, 0x01);
...
/*oppo hufeng 20150515 add for device list*/
            			if(deviceInfo_register_value==0x00){
                			register_device_proc("Cam_b", DEVICE_VERSION_IMX179, DEVICE_MANUFACUTRE_IMX179);
...
#ifdef VENDOR_EDIT
//jindian.guan add for otp
int get_otp_af(char * af)
{
    if (NULL==af) {
        return ERROR_INVALID_PARA;
    }
  	if (((AF[3]<<8)|AF[2]+110)>=((AF[1]<<8)|AF[0])) {
...
static int read_otp_af(void)
{
    int temp;
    //kal_uint8 retry = 3;
    //kal_uint8 PAGE_AF_OTPDATA = 9;
    //kal_uint8 AF_otpdata_valid= false;
	kal_uint8 flag1=0,flag2=0,flag3=0;

    
        write_cmos_sensor(0x3380, 0x28); // Set ECC
        write_cmos_sensor(0x3400, 0x01); // read ON
        write_cmos_sensor(0x3402, 0x01); //Select Page
		flag1= read_cmos_sensor(0x3423);
		flag2= read_cmos_sensor(0x342A);
		flag3= read_cmos_sensor(0x3431);
		if(flag3==1){
		    AF[0]=(u8)(read_cmos_sensor(0x3432));
            AF[1]=(u8)(read_cmos_sensor(0x3433));
            AF[2]=(u8)(read_cmos_sensor(0x3434));
            AF[3]=(u8)(read_cmos_sensor(0x3435));
		}else if(flag3==0&&flag2==1){
		    AF[0]=(u8)(read_cmos_sensor(0x342B));
            AF[1]=(u8)(read_cmos_sensor(0x342C));
            AF[2]=(u8)(read_cmos_sensor(0x342D));
            AF[3]=(u8)(read_cmos_sensor(0x342E));
		}else{
			AF[0]=(u8)(read_cmos_sensor(0x3424));
            AF[1]=(u8)(read_cmos_sensor(0x3425));
            AF[2]=(u8)(read_cmos_sensor(0x3426));
            AF[3]=(u8)(read_cmos_sensor(0x3427));
		}
		LOG_INF("flag1=%d,flag2=%d,flag3=%d",flag1,flag2,flag3);
		LOG_INF("imx179 af data:inf_h=0x%x, inf_l=0x%x,macro_h=0x%x,macro_l=0x%x\n",AF[3],AF[2],AF[1],AF[0]);
...
static int check_otp_awb(uint32_t index)        //zl:check flag
{
    unsigned char  flag = 0, id = 0,page=0;
	...
	write_cmos_sensor(0x0100, 0x00);
    write_cmos_sensor(0x3832, 0x1C); // 24MHz Set OTP_Write CLK , 60us=data/Inclk , MCLK 
    write_cmos_sensor(0x3833, 0x02);
    write_cmos_sensor(0x3868, 0x00); // Set MCLK data=mclk*256
    write_cmos_sensor(0x3869, 0x09);

    write_cmos_sensor(0x3380, 0x28); // Set ECC

    write_cmos_sensor(0x3400, 0x01); // read ON
	if(page ==0)
    write_cmos_sensor(0x3402, page); //Select Page
	else
	write_cmos_sensor(0x3402, page-1); //Select Page

    mdelay(10);

    // read flag
	if(page == 1){
    flag = read_cmos_sensor(0x3424);
    id = read_cmos_sensor(0x3425);
    }
	else{
	flag = read_cmos_sensor(0x3404);
    id = read_cmos_sensor(0x3405);
       }
    LOG_INF("imx179_otp check_otp_flag:reg 0x3404 page=%d  flag=%x  vendorid=%x\n",page,flag,id);
...
static uint32_t read_otp_wb_imx179(uint32_t index, struct otp_struct* otp)
{
    ...
    if(index ==1)
        start_address=0x3425;
    else
        start_address=0x3405;
    temp=read_otp(index, start_address, arr, 26);

    if (temp==1) {
        uint32_t current_gh,current_rh,current_bh,current_gl,current_rl,current_bl;
        uint32_t current_g,current_r,current_b;
        ...
        otp->rg_ratio =(arr[9]<<8)+arr[8];
        otp->bg_ratio =(arr[11]<<8)+arr[10];


    }
    else {
        LOG_INF(" imx179_otp read all otp fail\n");
        return 0;
    }
    return 1;
}

static int imx179_update_awb_gain(uint32_t R_GAIN,uint32_t G_GAIN,uint32_t B_GAIN)//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{

        LOG_INF("imx179_otp R_GAIN=0x%x G_GAIN=0x%x, B_GAIN=0x%x\n", R_GAIN, G_GAIN, B_GAIN);
        ...
        write_cmos_sensor(0x0214, G_GAIN>>8);    //zl:why rewrite?
        write_cmos_sensor(0x0215, G_GAIN& 0xFF);
        LOG_INF("imx179_otp ok\n");
        return 0;
}
...
static int imx179_update_otp_wb(void)
{
    ...
    LOG_INF(" imx179_otp update_otp_wb++\n");
    for (i=0;i<3;i++) {
    ...
    if (i==3){
        ...
        LOG_INF("imx179_otp  no valid wb OTP data\n");
        return 1;
       }
    memset(&current_otp, 0, sizeof(current_otp));
    wb_flag = read_otp_wb_imx179(otp_index, &current_otp);
    if (true == wb_flag){
        //2016/04/15 lizhiqian modify for distinguish diffrent module QTECH or SUNNY
        LOG_INF(" imx179_otpcurrent_otp->customer_id = 0x%x\n",current_otp.customer_id);
        if (current_otp.customer_id == 0x05) {
            RG_Ratio_typical = RGr_Ratio_Typical_QTECH;
            BG_Ratio_typical = BGr_Ratio_Typical_QTECH;
            //GG_Ratio_typical = GG_Ratio_typical_QTECH;
        }
        else if (current_otp.customer_id == 0x01){
            RG_Ratio_typical = RGr_Ratio_Typical_SUNNY;
            BG_Ratio_typical = BGr_Ratio_Typical_SUNNY;
        }
        else {
            LOG_INF(" imx179_otp it is not QTECH or SUNNY module = 0x%x\n",current_otp.customer_id);
            return 1;
        }
        //calculate gain
        //0x400 = 1x gain
        LOG_INF(" imx179_otp current_otp.rg_ratio = 0x%x\n",current_otp.rg_ratio);
        LOG_INF(" imx179_otp current_otp.bg_ratio = 0x%x\n",current_otp.bg_ratio);
        LOG_INF(" imx179_otp RG_Ratio_typical = 0x%x\n",RG_Ratio_typical);
        LOG_INF(" imx179_otp BG_Ratio_typical = 0x%x\n",BG_Ratio_typical);
        if (current_otp.rg_ratio==0||current_otp.bg_ratio==0){
            LOG_INF("imx179_otp failed,current_otp.rg_ratio==0 ||current_otp.bg_ratio==0\n ");
            return 1;
        }
#define standard_value  256
        r_ratio = standard_value * RG_Ratio_typical / current_otp.rg_ratio;
        b_ratio = standard_value * BG_Ratio_typical / current_otp.bg_ratio;
        if (r_ratio==0||b_ratio==0){
            LOG_INF("imx179_otp failed,r_ratio==0 ||b_ratio==0\n ");
            return 1;
        }

        if (r_ratio >= standard_value ) {
            if (b_ratio >= standard_value) {
                R_GAIN = (USHORT)(GAIN_DEFAULT * r_ratio / standard_value);
                G_GAIN = GAIN_DEFAULT;
                B_GAIN = (USHORT)(GAIN_DEFAULT * b_ratio / standard_value);
            }
            else {
                R_GAIN =  (USHORT)(GAIN_DEFAULT*r_ratio / b_ratio );
                G_GAIN = (USHORT)(GAIN_DEFAULT*standard_value / b_ratio );
                B_GAIN = GAIN_DEFAULT;
            }
        }
        else {
            if (b_ratio >= standard_value) {
                R_GAIN = GAIN_DEFAULT;
                G_GAIN = (USHORT)(GAIN_DEFAULT*standard_value /r_ratio);
                B_GAIN =  (USHORT)(GAIN_DEFAULT*b_ratio / r_ratio );
            }
            else {
                Gr_GAIN = (USHORT)(GAIN_DEFAULT*standard_value/ r_ratio );
                Gb_GAIN = (USHORT)(GAIN_DEFAULT*standard_value/b_ratio );
                if (Gr_GAIN >= Gb_GAIN) {
                    R_GAIN = GAIN_DEFAULT;
                    G_GAIN = (USHORT)(GAIN_DEFAULT *standard_value/ r_ratio );
                    B_GAIN =  (USHORT)(GAIN_DEFAULT*b_ratio / r_ratio );
        ...
        imx179_update_awb_gain(R_GAIN,G_GAIN,B_GAIN);
    }
    else {
        LOG_INF("imx179_otp read faile %d\n",__LINE__);
        return 1;
    }
    LOG_INF("update_otp_wb--\n");
    return 0;
}
static int read_otp(uint32_t page, uint32_t address, uint32_t* array, uint32_t size)
{
    ...
    write_cmos_sensor(0x3380, 0x08); // Set ECC
    ...
    mdelay(10);
    for (i=0;i<size;i++) {
        *(array+i)=read_cmos_sensor(address+i);
            LOG_INF(" imx179_otp read_from_otp [%d] == 0x%x\n",i,array[i]);
    }
    LOG_INF(" imx179_otp page = 0x%x\n",page);
