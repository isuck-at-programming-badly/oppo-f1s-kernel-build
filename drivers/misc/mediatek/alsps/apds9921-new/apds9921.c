...
int apds9921_read_als(struct i2c_client *client, u32 *data, bool calied);
int apds9921_i2c_master_operate(struct i2c_client *client, const char *buf, int count, int i2c_flag);
int apds9921_calibration_ps(struct i2c_client *client);
static int apds9921_init_client(struct i2c_client *client);
static void apds9921_set_ps_threshold(struct i2c_client *client, int low_threshold, int high_threshold);
/*----------------------------------------------------------------------------*/
...
static int apds9921_set_psensor_threshold(struct i2c_client *client);
...
struct ps_adjust_para *apds9921_ps_adjust_para_real = NULL;
...
//static struct platform_driver apds9921_alsps_driver;
static int apds9921_get_ps_value(struct apds9921_priv *obj, u16 ps);
static int apds9921_get_first_ps_value(struct apds9921_priv *obj, u16 ps);
...
static int  apds9921_dump_reg(void)
{
	int i;
	int res = 0;
	u8 databuf[3];
	...
	APS_ERR("####apds9921_dump_reg start:####\n%s\n[psensord]####apds9921_dump_reg end:####\n",_buff);
	return 1;
}

static int apds9921_esd_debug(void){
	if(!apds9921_obj)
	{
		APS_ERR("%s:apds9921_obj is null!!\n",__func__);
		return 0;
	}

	apds9921_read_ps(apds9921_obj->client, &apds9921_obj->ps);
	apds9921_read_als(apds9921_obj->client, &apds9921_obj->als,true);
	APS_ERR("####%s:als:%d ps:%d\n",__func__,apds9921_obj->als,apds9921_obj->ps);
	apds9921_dump_reg();
	return 0;
}
#endif
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_show_cali(struct device_driver *ddri, char *buf)
{
	if(!apds9921_obj)
	{
		APS_ERR("apds9921_obj is null!!\n");
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_store_cali(struct device_driver *ddri, const char *buf, size_t count)
{
	int calib;
	if(!apds9921_obj)
	{
	...
	{ 
		if (calib==1)
			{
				apds9921_obj->cali=apds9921_calibration_ps(apds9921_obj->client);
			}				
	}
	else
	{
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_show_offset(struct device_driver *ddri, char *buf)
{
	...
	databuf_L[0]= APDS9921_DD_PRX_CAN_0_ADDR;
	res = apds9921_i2c_master_operate(apds9921_obj->client, databuf_L, 0x101, I2C_FLAG_READ);
	...
	databuf_H[0]= APDS9921_DD_PRX_CAN_1_ADDR;
	res = apds9921_i2c_master_operate(apds9921_obj->client, databuf_H, 0x101, I2C_FLAG_READ);
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_store_offset(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{ 
       	databuf[0] = APDS9921_DD_PRX_CAN_0_ADDR;
		databuf[1] = offsetL;
		res = apds9921_i2c_master_operate(apds9921_obj->client, databuf, 0x2, I2C_FLAG_WRITE);//xtalk cancellation
		if (res <= 0) {
		 goto EXIT_ERR;
		}

		databuf[0] = APDS9921_DD_PRX_CAN_1_ADDR;
		databuf[1] = offsetH;
		res = apds9921_i2c_master_operate(apds9921_obj->client, databuf, 0x2, I2C_FLAG_WRITE);//xtalk cancellation
		if (res <= 0) {
		 goto EXIT_ERR;
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_show_gain(struct device_driver *ddri, char *buf)
{
	if(!apds9921_obj)
	{
		APS_ERR("apds9921_obj is null!!\n");
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_store_gain(struct device_driver *ddri, const char *buf, size_t count)
{
	if(!apds9921_obj)
	{
		APS_ERR("apds9921_obj is null!!\n");
	...
	if(1 != sscanf(buf, "%d", &apds9921_obj->gain))
	{
		APS_ERR("invalid content: '%s', length = %zd\n", buf, count);
	}
	return count;
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_show_threshold(struct device_driver *ddri, char *buf)
{
	int threshold_hi = 0;
	int threshold_lo = 0;
	int val_ps_cali = 0;
	...
	threshold_lo = atomic_read(&apds9921_obj->ps_thd_val_low);
	threshold_hi = atomic_read(&apds9921_obj->ps_thd_val_high);
	val_ps_cali = apds9921_obj->ps_cali;
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_store_threshold(struct device_driver *ddri, const char *buf, size_t count)
{
	int threshold_hi = 0;
	int threshold_lo = 0;
	if(!apds9921_obj)
	...
	APS_ERR("%s set threshold low: %d, high: %d, cali: %d\n", __func__, threshold_lo,threshold_hi,apds9921_obj->ps_cali);
	atomic_set(&apds9921_obj->ps_thd_val_low,  (threshold_lo+apds9921_obj->ps_cali));
	atomic_set(&apds9921_obj->ps_thd_val_high,  (threshold_hi+apds9921_obj->ps_cali));//need to confirm
	apds9921_set_psensor_threshold(apds9921_obj->client);
...
static ssize_t apds9921_show_als(struct device_driver *ddri, char *buf)
{
	...
	if((res = apds9921_read_als(apds9921_obj->client, &als,false)))
	{
		return scnprintf(buf, PAGE_SIZE, "ERROR: %d\n", res);
	}
	else
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_show_ps_adjust_para(struct device_driver *ddri, char *buf)
{
	static char tmpbuf[500] = {0};
	int res;
	APS_FUN();
	res = prox_show_adjust_para(apds9921_ps_adjust_para_real,tmpbuf);
	if(res == -2){
		APS_ERR("apds9921_ps_adjust_para_real is null!!\n");
	}else{
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_store_ps_adjust_para(struct device_driver *ddri, const char *buf, size_t count)
{
	int enable,res;
	int index,value;
	APS_FUN();
	...
	enable = test_bit(CMC_BIT_PS, &apds9921_obj->enable) ? (1) : (0);

	if(2 == sscanf(buf, "%d %d", &index,&value))
	{
		if(enable == 0){
			res = prox_set_adjust_para(apds9921_ps_adjust_para_real,index,value);
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_show_tuning(struct device_driver *ddri, char *buf)
{
	u8 databuf[2];
	int res = 0;
...
/*----------------------------------------------------------------------------*/
static ssize_t apds9921_store_tuning(struct device_driver *ddri, const char *buf, size_t count)
{
	u8 databuf[2];
	int res = 0;
	int idx,value;
...
static DRIVER_ATTR(cali,      S_IWUSR | S_IRUGO, apds9921_show_cali,  apds9921_store_cali );
static DRIVER_ATTR(offset,      S_IWUSR | S_IRUGO, apds9921_show_offset,  apds9921_store_offset );
static DRIVER_ATTR(gain_als,      S_IWUSR | S_IRUGO, apds9921_show_gain,  apds9921_store_gain);
static DRIVER_ATTR(threshold,      S_IWUSR | S_IRUGO, apds9921_show_threshold,  apds9921_store_threshold);
static DRIVER_ATTR(als,      S_IWUSR | S_IRUGO, apds9921_show_als,    NULL);
static DRIVER_ATTR(ps_adjust_para,      S_IWUSR | S_IRUGO, apds9921_show_ps_adjust_para,  apds9921_store_ps_adjust_para );
static DRIVER_ATTR(tuning,      S_IWUSR | S_IRUGO, apds9921_show_tuning,  apds9921_store_tuning );
...
static void apds9921_power(struct alsps_hw *hw, unsigned int on) 
{
	...
	if (hw->power_id != POWER_NONE_MACRO) {
		if (power_on == on) {
			...
			if(!hwPowerOn(hw->power_id, hw->power_vol, "APDS9921")) {
				APS_ERR("power on fails!!\n");
			}
		} else {
			if(!hwPowerDown(hw->power_id, "APDS9921")) {
	...
	if (enable) {
		...
		if(enable_count<=0){
			...
			if (test_bit(CMC_BIT_PS, &obj->enable)==0)
			{
				...
				databuf[1] = APDS9921_DD_ALS_MEAS_RES_16_BIT|APDS9921_DD_ALS_MEAS_RATE_25_MS;
				res = apds9921_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
				if (res <= 0) {
					APS_LOG("set APDS9921_DD_ALS_MEAS_RATE_ADDR i2c fail\n");
					goto EXIT_ERR;
				}
...
static long apds9921_enable_ps(struct i2c_client *client, int enable)
{
	...
	if (enable) {
		...
		main_ctrl = databuf[0]|APDS9921_DD_PRX_EN|APDS9921_DD_ALS_EN;
		...
		//apds9921_set_ps_threshold(obj->client,80, 120);
		...
		ps_value = apds9921_get_first_ps_value(obj, obj->ps);
		//APS_LOG("apds9921_enable_ps enable_ps ps_value=%d\n",ps_value);
		ps_report_interrupt_data(ps_value);
		APS_LOG("apds9921_enable_ps ps=%d, ps_value=%d\n",obj->ps,ps_value);
...
static void apds9921_esd_handle(struct i2c_client *client, int esd_type)
{
    ...
    if(esd_type == 1)    // intflag err
    {
	 ...
	 if(als_state > 0)
		{
		    apds9921_enable_als(client, 0);
        	}
        if(ps_state  > 0)
        	{
		    apds9921_enable_ps(client, 0);
		}
		...
		
	{
	 ...
	 if(als_state > 0)
		{
		    apds9921_enable_als(client, 1);
       	}
        if(ps_state > 0)
       	{
		    apds9921_enable_ps(client, 1);
		}        
...
static int apds9921_check_intr(struct i2c_client *client) 
{
	...
	// kk 13-May-2015
	if(0 == (buffer[0] & APDS9921_DD_PRX_LOGICAL_STATUS)) {
		obj->ps_state = 1; // far		
	}
	else {
		obj->ps_state = 0; // near		
...
{
	struct apds9921_priv *obj = g_apds9921_ptr;
	struct alsps_hw *hw = apds9921_get_cust_alsps_hw();
...
int apds9921_setup_eint(struct i2c_client *client)
{
	...
	if (request_irq(irq, apds9921_interrupt, IRQF_TRIGGER_FALLING, "apds9921", (void *)client))
	{
	...
	//mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_TYPE, apds9921_eint_func, 0);
...
static int apds9921_init_client(struct i2c_client *client)
{
	...
	{
		if (get_project() == 15127 || get_project() == 15128 || get_project() == 15129 || get_project() == 15130
			|| get_project() == 15131 || get_project() == 15132 || get_project() == 15133 || get_project() == 15134)
			databuf[1] = APDS9921_DD_LED_FREQ_60_KHZ|APDS9921_DD_PRX_DEFAULT_LED_CURRENT;
		else
			databuf[1] = APDS9921_DD_LED_FREQ_80_KHZ|APDS9921_DD_PRX_DEFAULT_LED_CURRENT;
	}
	else
		databuf[1] = APDS9921_DD_LED_FREQ_100_KHZ|APDS9921_DD_PRX_DEFAULT_LED_CURRENT;
	#endif
	res = apds9921_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	if (res <= 0) {
		goto EXIT_ERR;
	...
	if (alsps_dev == ALSPS_APDS_9922)
	{
		if (get_project() == 16021 || get_project() == 16022 || get_project() == 16023 || get_project() == 16024)
			databuf[1] = 28;
		else if (get_project() == 15127 || get_project() == 15128 || get_project() == 15129 || get_project() == 15130
			|| get_project() == 15131 || get_project() == 15132 || get_project() == 15133 || get_project() == 15134)
			databuf[1] = 28;
		else databuf[1] = 32;
	}
	...
	databuf[1] = 0x40|APDS9921_DD_PRX_DEFAULT_RES|APDS9921_DD_PRX_MEAS_RATE_6_25_MS;
	res = apds9921_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	if (res <= 0) {
		goto EXIT_ERR;
	}*/
	...
	databuf[1] = APDS9921_DD_PRX_PERS_1; //lgc-2015-06-28
	res = apds9921_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	if (res <= 0) {
		goto EXIT_ERR;
	}

	databuf[0] = APDS9921_DD_ALS_THRES_UP_ADDR;
	...
	if (0 == obj->hw->polling_mode_ps) {
		if (1 == ps_cali.valid) {
			...
			apds9921_set_psensor_threshold(client);
		}
	}
...
{	 
	...
	als = ((((long)als_value*APDS9921_DD_LUX_FACTOR*factor*hw->als_gain1)<<1)/((long)als_time*als_gain*hw->als_gain2)+1)>>1;
	if (get_project() == 16034){
		als = als * 14 / 10;
	}
	/*
	if(calied){
		*data = ((((long)als_value*APDS9921_DD_LUX_FACTOR*factor*hw->als_gain1*apds9921_obj->gain)<<1)/((long)als_time*als_gain*hw->als_gain2*1000)+1)>>1;
		//APS_LOG("apds9921_read_als calied   lux = %d,raw %d[%d,%d,%d,%d/%d,%d,%d,%d]\n", *data,als_value,APDS9921_DD_LUX_FACTOR,factor,hw->als_gain1,apds9921_obj->gain,als_time,als_gain,hw->als_gain2,1000);
	}else{
		*data = ((((long)als_value*APDS9921_DD_LUX_FACTOR*factor*hw->als_gain1)<<1)/((long)als_time*als_gain*hw->als_gain2)+1)>>1;
		//APS_LOG("apds9921_read_als uncalied lux = %d,raw %d[%d,%d,%d/%d,%d,%d]\n", *data,als_value,APDS9921_DD_LUX_FACTOR,factor,hw->als_gain1,als_time,als_gain,hw->als_gain2);
	}*/
...
#ifdef VENDOR_EDIT
//ye.zhang@BSP.Sensors.Performance,2016-08-25
//modify for 9922 and read code clearly
	if ((get_project() == 15127 || get_project() == 15128 || get_project() == 15129 || get_project() == 15130)
		&& (alsps_dev == ALSPS_APDS_9922))
		als = als * 5 / 8;
#ifdef VENDOR_EDIT
/*xiaohua.tian@Prd6.BaseDrv.Sensor,2016/09/26 modify origin als for 16321*/
	else if (get_project() == 16021 && (alsps_dev == ALSPS_APDS_9922))
		als = als * 8 / 10;
#endif
	//APS_ERR("als=%d  apds9921_obj->gain=%d\n",als,apds9921_obj->gain);
	if(calied){
		if ((apds9921_obj->gain > 0) && (apds9921_obj->gain != 1000))
		{
			als *= apds9921_obj->gain;

			if (alsps_dev == ALSPS_APDS_9922)
			{
				if (get_project() == 15111 || get_project() == 15112 || get_project() == 15113 || get_project() == 15114 || get_project() == 16031 || get_project() == 16034)
					als /=  1250;
				else
					als /= 1000;
				//als need divide by 1.1 after calibration
...
long apds9921_read_ps(struct i2c_client *client, u16 *data)
{
	...
	//APS_LOG("apds9921_read_ps ps_data=%d", ps_data);
...
static int apds9921_get_first_ps_value(struct apds9921_priv *obj, u16 ps)
{
	...
	if (!invalid) {
		...
		APS_LOG("apds9921_get_ps_value 222  val  = %d",val);
		return -1;
	}
}
...
static void apds9921_eint_work(struct work_struct *work)
{
	...
	if ((err = apds9921_check_intr(obj->client))) {
		...
		//get raw data
		apds9921_read_ps(obj->client, &obj->ps);
		apds9921_read_als(obj->client, &obj->als,true);
		// kk 13-May-2015
		...
		//APS_ERR("apds9921 int top half time = %lld\n", int_top_time);
			if(obj->als<HIGHLIGHT_MODE_ALS_THRESHOLD||obj->ps<900){
				ps_value = apds9921_get_ps_value(obj, obj->ps);
				ps_report_interrupt_data(ps_value);
				APS_ERR("report ps=%d\n", ps_value);
				/*
				...
				APS_LOG("apds9921_eint_work report ps=%d\n", sensor_data.values[0] );			
...
int	apds9921_calibration_ps(struct i2c_client *client)
{
	...
	databuf[0] = APDS9921_DD_PRX_CAN_0_ADDR;
	databuf[1] = 0x00;
	res = apds9921_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);//xtalk cancellation
	if (res <= 0) {
		goto EXIT_ERR;
	}
	...
	APS_ERR("APDS9921_PS_CAL pdata = %d\n", ps_data);

	if (ps_data > APDS9921_PS_CAL_CROSSTALK_HIGH&&ps_data <= 900){
		ps_offset = ps_data - APDS9921_PS_CAL_CROSSTALK_HIGH;
...
EXIT_ERR:
//	mt_eint_unmask(CUST_EINT_ALS_NUM);
	APS_ERR("apds9921_i2c_master_operate\n");
	return -1;
}
...
//ye.zhang@BSP.PSW, 2016-04-13, delete for set PS threshold at a time
	...
	//APS_ERR("apds9921_set_psensor_threshold function high: 0x%x, low:0x%x\n", atomic_read(&obj->ps_thd_val_high), atomic_read(&obj->ps_thd_val_low));
	...
	//APS_ERR("apds9921_set_psensor_threshold 222 function high: 0x%x, low:0x%x\n", atomic_read(&obj->ps_thd_val_high), atomic_read(&obj->ps_thd_val_low));
#else//VENDOR_EDIT
	u8 databuf[5];
	int res = 0;
	...
	res = apds9921_i2c_master_operate(client, databuf, 0x5, I2C_FLAG_WRITE);
	if (res <= 0) {
		return -1;
	}
#endif//VENDOR_EDIT
...
static void apds9921_set_ps_threshold(struct i2c_client *client, int low_threshold, int high_threshold)
{
    ...
    apds9921_set_psensor_threshold(obj->client);
}
...
static long apds9921_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case ALSPS_GET_PS_DATA:     
			...
			/*buffer[0] = APDS9921_DD_PRX_MEAS_RATE_ADDR;
			res = apds9921_i2c_master_operate(obj->client, buffer, 0x101, I2C_FLAG_READ);
			if (res <= 0) {
				APS_ERR("apds i2c error");
			}
			buffer[2]=buffer[0] ;
			APS_LOG("APDS9921_DD_PRX_MEAS_RATE_ADDR = 0X%x\n", buffer[2] );*/
			#if APDS_DEBUG
			dump_count++;
			...
			buffer[0] = APDS9921_DD_ALS_THRES_UP_ADDR;
			res = apds9921_i2c_master_operate(obj->client, buffer, 0x101, I2C_FLAG_READ);
			if (res <= 0) {
				APS_ERR("apds i2c error");
			}
			buffer[3]=buffer[0] ;
			#if APDS_DEBUG
			//APS_LOG("APDS9921_DD_ALS_THRES_UP_ADDR = 0X%x\n", buffer[3] );
			#endif
		      // if(buffer[2] != 0x54 || buffer[3]!=0x40)
			if(buffer[3]==0xff)  	
				apds9921_esd_handle(obj->client,1);
			...
			if( ((obj->ps<atomic_read(&obj->ps_thd_val_low))&&(obj->ps_state==0))
				|| ((obj->ps>atomic_read(&obj->ps_thd_val_high))&&(obj->ps_state==1)) ){
					...
					if( ((obj->ps<atomic_read(&obj->ps_thd_val_low))&&(obj->ps_state==0))
						|| ((obj->ps>atomic_read(&obj->ps_thd_val_high))&&(obj->ps_state==1)) ){
						APS_ERR("apds9921 int err ps raw:%d state:%d\n",obj->ps,obj->ps_state);
						apds9921_esd_handle(obj->client,1);
						goto err_out;
					}
			}
		...
		case ALSPS_GET_ALS_DATA: 
			if ((err = apds9921_read_als(obj->client, &obj->als,true))) {
				goto err_out;
			}
		...
		case ALSPS_GET_ALS_RAW_DATA:    
			...
			//APS_LOG("apds9921 ALSPS_GET_ALS_RAW_DATA = %d \n",dat);
			break;
...
//ye.zhang@BSP, remove for decrease log number
	     		 switch (set_ps_thd_para.algo_state) {
	      		 case PS_ADJUST_TREND_STATE : 
	                    APS_LOG("apds9921_set_ps_threshold ps average:%d state: TREND\n", set_ps_thd_para.ps_average);
	                    break;
	                case PS_ADJUST_NOTREND_STATE :
	                    APS_LOG("apds9921_set_ps_threshold ps average:%d state: NOTREND\n", set_ps_thd_para.ps_average);
	                    break;
	                case PS_ADJUST_HIGHLIGHT_STATE :
	                    APS_LOG("apds9921_set_ps_threshold ps average:%d state: HIGHLIGHT\n", set_ps_thd_para.ps_average);
	                    break;
	                case PS_ADJUST_AVOID_DIRTY_STATE : 
	                    APS_LOG("apds9921_set_ps_threshold ps average:%d state: AVOID_DIRTY\n", set_ps_thd_para.ps_average);
	                    break;
	                default:
	                    APS_LOG("apds9921_set_ps_threshold ps average:%d state: impossible\n", set_ps_thd_para.ps_average);
	                    break;
	            }
#endif//VNDOR_EDIT
	            apds9921_set_ps_threshold(obj->client,
	        ...
	        case ALSPS_GET_CUST_PS_ADJUST_PARA: //0x22 lycan add for
	            	...
	            	hw = apds9921_get_cust_alsps_hw();
	            	if(copy_to_user(ptr, apds9921_ps_adjust_para_real, sizeof( struct ps_adjust_para)))
	           	 {
	               	 err = -EFAULT;
	                	goto err_out;
	            	}              
		...
		case ALSPS_GET_HIGHLIGHT_DATA: 
			if(obj->als<HIGHLIGHT_MODE_ALS_THRESHOLD)
				dat=0;
			/*buffer[0] = APDS9921_DD_INT_PERSISTENCE_ADDR;
			buffer[1] = APDS9921_DD_PRX_PERS_1; //lgc-2015-06-28
			res = apds9921_i2c_master_operate(client, buffer, 0x2, I2C_FLAG_WRITE);
			if (res <= 0) {
				goto  err_out;
			}*/
			else 
				dat=1;
			/*buffer[0] = APDS9921_DD_INT_PERSISTENCE_ADDR;
			buffer[1] = APDS9921_DD_PRX_PERS_5; //lgc-2015-06-28
			res = apds9921_i2c_master_operate(client, buffer, 0x2, I2C_FLAG_WRITE);
			if (res <= 0) {
				goto  err_out;
			}*/
	...
	switch (command) {
		...
		case SENSOR_GET_DATA:
			if ((buff_out == NULL) || (size_out< sizeof(hwm_sensor_data))) {
				...
				APS_ERR("apds9921 als SENSOR_GET_DATA = %d \n",sensor_data->values[0] );
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
...
static int apds9921_als_open_report_data(int open)
{
	return 0;
}

static int apds9921_als_enable_nodata(int en)
{
	struct apds9921_priv *obj = i2c_get_clientdata(apds9921_i2c_client);
	int err = 0;
	if (en) {
		if ((err = apds9921_enable_als(obj->client, 1))) {
			APS_ERR("enable als fail: %d\n", err);
...
static int apds9921_als_set_delay(u64 ns)
{
	return 0;
}

static int apds9921_als_get_data(int* value, int* status)
{
	int res;
	u8 buffer[4];
	u32 b[2];
	...
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	...
	if(first_als_data){
		if((first_als_data>5) && (test_bit(CMC_BIT_PS, &obj->enable)==0)){
			buffer[0] = APDS9921_DD_ALS_MEAS_RATE_ADDR;
			buffer[1] = APDS9921_DD_ALS_MEAS_RES_18_BIT|APDS9921_DD_ALS_MEAS_RATE_100_MS;;
			apds9921_i2c_master_operate(obj->client, buffer, 0x2, I2C_FLAG_WRITE);
...
static int apds9921_ps_open_report_data(int open)
{
	return 0;
}

static int apds9921_ps_enable_nodata(int en)
{
	int err = 0;
	struct apds9921_priv *obj = i2c_get_clientdata(apds9921_i2c_client);
...
static int apds9921_ps_set_delay(u64 ns)
{
	return 0;
}

static int apds9921_ps_get_data(int* value, int* status)
{   
	struct apds9921_priv *obj = i2c_get_clientdata(apds9921_i2c_client);
	apds9921_read_ps(obj->client, &obj->ps);
	APS_ERR("apds9921_ps_get_data ps data=%d!\n",obj->ps);
...
static int apds9921_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	apds9921_obj = obj;
	obj->hw = apds9921_get_cust_alsps_hw();
	...
	buffer[0]=APDS9921_DD_PART_ID_ADDR;
	err = apds9921_i2c_master_operate(client, buffer, 0x301, I2C_FLAG_READ);
	if (err <= 0)
	{
		APS_ERR("apds9921_device read device ID failed\n");
	...
	switch(buffer[0]){
		case 0xB3:
			...
			if(hw->sec_para_supported){
				apds9921_ps_adjust_para_real=hw->p_ps_adjust_para2;
			}else{
				apds9921_ps_adjust_para_real=hw->p_ps_adjust_para;
			}
			break;
		default:
...
#endif /*VENDOR_EDIT*/
	if (apds9921_ps_adjust_para_real != NULL)
	{
		atomic_set(&obj->ps_thd_val_high,  apds9921_ps_adjust_para_real->ps_thd_high_highlight);
		atomic_set(&obj->ps_thd_val_low,  apds9921_ps_adjust_para_real->ps_thd_low_highlight);
	}
	...
	als_ctl.open_report_data= apds9921_als_open_report_data;
	als_ctl.enable_nodata = apds9921_als_enable_nodata;
	als_ctl.set_delay  = apds9921_als_set_delay;
	...
	als_data.get_data = apds9921_als_get_data;
	...
	ps_ctl.open_report_data= apds9921_ps_open_report_data;
	ps_ctl.enable_nodata = apds9921_ps_enable_nodata;
	ps_ctl.set_delay  = apds9921_ps_set_delay;
	...
	ps_data.get_data = apds9921_ps_get_data;
	...
	else register_device_proc("Sensor_alsps", "apds9921", "AVAGOTECH");
#endif /*VENDOR_EDIT*/
...
/*----------------------------------------------------------------------------
static int apds9921_probe(struct platform_device *pdev) 
{
	struct alsps_hw *hw = apds9921_get_cust_alsps_hw();

	apds9921_power(hw, 1);
	//apds9921_force[0] = hw->i2c_num;
	//apds9921_force[1] = hw->i2c_addr[0];
	//APS_DBG("I2C = %d, addr =0x%x\n",apds9921_force[0],apds9921_force[1]);
...
static int __init apds9921_init(void)
{
	...
	i2c_register_board_info(hw->i2c_num, &i2c_apds9921, 1);
...
{
	APS_FUN();
	//platform_driver_unregister(&apds9921_alsps_driver);
}
