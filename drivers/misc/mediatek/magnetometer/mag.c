	...
	for(i = 0; i < MAX_M_V_SENSOR; i++)
	{
	   ...
	   if(!(cxt->active_data_sensor&(0x01<<i)))
	   {
		...
		if(ID_M_V_MAGNETIC ==i)
		{
			...
			if ((cur_ns - m_pre_ns) >= delay_ms*1800000LL)
			{
				//VENDOR_EDIT zhihong.lu@BSP.sensor remove to avoid too much data report
				MAG_ERR("mag_data_report is late\n");
				/*
		...
		if(ID_M_V_ORIENTATION ==i)
		{
			...
			{
				//VENDOR_EDIT zhihong.lu@BSP.sensor remove to avoid too much data report
				MAG_ERR("ori_data_report is late\n");
				/*
    ...
    if(1 == enable)
    {
	   ...
	   if((0!=cxt->active_data_sensor) && (false == cxt->is_polling_run)&&(false == cxt->is_batch_enable))
	   {
	      if(false == cxt->mag_ctl.is_report_input_direct)
	      {
				...
				//VENDOR_EDIT zhihong.lu@BSP.sensor
				MAG_ERR("%s[%d]:delay:%d\n" ,__func__,__LINE__,atomic_read(&cxt->delay));
                                  ...
                                  const char *buf, size_t count)
{
  //  struct mag_context *devobj = (struct mag_context*)dev_get_drvdata(dev);
	...
	if(false == cxt->mag_ctl.is_report_input_direct)
    {
		...
		//VENDOR_EDIT zhihong.lu@BSP.sensor
		MAG_ERR("%s[%d]:delay:%d\n" ,__func__,__LINE__,atomic_read(&mag_context_obj->delay));
    }
                                  ...
                                  const char *buf, size_t count)
{
	...
	mutex_unlock(&mag_context_obj->mag_op_mutex);
	MAG_LOG(" mag_store_grbatch done: %d\n", cxt->is_batch_enable);
    	return count;
...
static int mag_input_init(struct mag_context *cxt)
{
	...
	input_set_capability(dev, EV_ABS, EVENT_TYPE_ROTVEC_W);
#endif
...
int mag_data_report(MAG_TYPE type,int x, int y, int z, int status, int64_t nt)
{
	...
	{
   		input_report_abs(cxt->idev, EVENT_TYPE_ROTVEC_W,status);
