...
/*----------------------------------------------------------------------------*/
#define DEBUG 1
#define MMC3530X_DEV_NAME         "mmc3530x"
#define DRIVER_VERSION          "1.0.0"
/*----------------------------------------------------------------------------*/
#define MMC3530X_DEBUG		1
#define MMC3530X_DEBUG_MSG	1
#define MMC3530X_DEBUG_FUNC	1
#define MMC3530X_RETRY_COUNT	3
#define MMC3530X_DEFAULT_DELAY	100
#define	MMC3530X_BUFSIZE  0x20
#define READMD			0


#if MMC3530X_DEBUG_MSG
#define MMCDBG(format, ...)	printk(KERN_INFO "mmc3530x " format "\n", ## __VA_ARGS__)
#else
#define MMCDBG(format, ...)
#endif

#if MMC3530X_DEBUG_FUNC
#define MMCFUNC(func) printk(KERN_INFO "mmc3530x " func " is called\n")
#else
#define MMCFUNC(func)
#endif
...
static int mmcd_delay = MMC3530X_DEFAULT_DELAY;
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_init_flag = 0;
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id mmc3530x_i2c_id[] = {{MMC3530X_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_mmc3530x={ I2C_BOARD_INFO(MMC3530X_DEV_NAME, (MMC3530X_I2C_ADDR>>1))};

/*the adapter id will be available in customization*/
//static unsigned short mmc3530x_force[] = {0x00, mmc3530X_I2C_ADDR, I2C_CLIENT_END, I2C_CLIENT_END};
//static const unsigned short *const mmc3530x_forces[] = { mmc3530x_force, NULL };
//static struct i2c_client_address_data mmc3530x_addr_data = { .forces = mmc3530x_forces,};
/*----------------------------------------------------------------------------*/
static int mmc3530x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id); 
static int mmc3530x_i2c_remove(struct i2c_client *client);
//static int mmc3530x_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info);

static int mmc3530x_local_init(void);
static int mmc3530x_remove(void);
extern struct mag_hw * mmc3530x_get_cust_mag_hw(void);

static struct mag_init_info mmc3530x_init_info = {
    .name = "mmc3530x",
    .init = mmc3530x_local_init,
    .uninit = mmc3530x_remove,
};
...
#define MMC3530X_DELAY_TM	10	/* ms */
#define MMC3530X_DELAY_SET	50	/* ms */
#define MMC3530X_DELAY_RST	50	/* ms */

#define MMC3530X_RESET_INTV	250
...
/*----------------------------------------------------------------------------*/
static struct i2c_driver mmc3530x_i2c_driver = {
    .driver = {
        .name  = MMC3530X_DEV_NAME,
    },
	.probe      = mmc3530x_i2c_probe,
	.remove     = mmc3530x_i2c_remove,
	//.detect     = mmc3530x_i2c_detect,
#if !defined(CONFIG_HAS_EARLYSUSPEND)
	.suspend    = mmc3530x_suspend,
	.resume     = mmc3530x_resume,
#endif 
	.id_table = mmc3530x_i2c_id,
	//.address_data = &mmc3530x_addr_data,
};

/*----------------------------------------------------------------------------*/
static atomic_t dev_open_count;

static int mmc3530x_SetPowerMode(struct i2c_client *client, bool enable)
{
 //modify when mmc3530 suspend do nothing
 return 0;
 
#if 0
	u8 databuf[2];    
	int res = 0;
	u8 addr = MMC3530X_REG_CTRL;
	//struct mmc3530x_i2c_data *obj = i2c_get_clientdata(client);

	if(hwmsen_read_byte(client, addr, databuf))
	{
		printk("mmc3530x: read power ctl register err and retry!\n");
		if(hwmsen_read_byte(client, addr, databuf))
	    {
		   printk("mmc3530x: read power ctl register retry err!\n");
		   return -1;
	    }
	}

	databuf[0] &= ~MMC3530X_CTRL_TM;
	...
	if(res <= 0)
	{
		printk("mmc3530x: set power mode failed!\n");
		return -1;
	}
	else
	{
		printk("mmc3530x: set power mode ok %x!\n", databuf[1]);
	}
...
/*----------------------------------------------------------------------------*/
static void mmc3530x_power(struct mag_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "mmc3530x")) 
			{
		...
		{
			if(!hwPowerDown(hw->power_id, "mmc3530x")) 
			{
...
static int I2C_RxData(char *rxData, int length)
{
	...
	struct mmc3530x_i2c_data *data = i2c_get_clientdata(client);
	char addr = rxData[0];
#endif
	...
	for(loop_i = 0; loop_i < MMC3530X_RETRY_COUNT; loop_i++)
	{
	...
	if(loop_i >= MMC3530X_RETRY_COUNT)
	{
		printk(KERN_ERR "%s retry over %d\n", __func__, MMC3530X_RETRY_COUNT);
		return -EIO;
	}
#if DEBUG
...
static int MMCECS_CheckDevice(void)
{
	char buffer[2];
	int ret;
	printk(" mmc3530x check device id\n");
	/* Set measure mode */
	buffer[0] = MMC3530X_REG_PRODUCTID_1;

	/* Read data */
	ret = I2C_RxData(buffer, 1);
	printk(" mmc3530x check device id = %x\n",buffer[0]);
	printk(" mmc3530x ret = %d\n",ret);
	if(ret < 0)
	{
		return ret;
	}
...
#if DEBUG	
	struct i2c_client *client = this_client;  
	struct mmc3530x_i2c_data *clientdata = i2c_get_clientdata(client);
#endif
...
#if 0	
	/* do RESET/SET every mmc3530X_RESET_INTV times read */
	if (!(read_idx % MMC3530X_RESET_INTV))
	{
		/* SET */
		data[0] = MMC3530X_REG_CTRL;
		data[1] = MMC3530X_CTRL_SET;
		/* not check return value here, assume it always OK */
		I2C_TxData(data, 2);
		//delay.tv_sec =0;
		//delay.tv_nsec = mmc3530X_DELAY_STDN *1000;
		//hrtimer_nanosleep(&delay, &aa, HRTIMER_MODE_REL, CLOCK_MONOTONIC);
		msleep(mmc3530X_DELAY_STDN);
	}
...
if (!(read_idx % MMC3530X_RESET_INTV))
	{
		/* RM */
			data[0] = MMC3530X_REG_CTRL;
			data[1] = MMC3530X_CTRL_REFILL;
			/* not check return value here, assume it always OK */
			I2C_TxData(data, 2);
			/* wait external capacitor charging done for next RM */
			msleep(MMC3530X_DELAY_SET);
		    data[0] = MMC3530X_REG_CTRL;
		    data[1] = MMC3530X_CTRL_SET;
		    I2C_TxData(data, 2);
		    msleep(1);
		    ...
		    msleep(1);
	            ...
	            data[1] = MMC3530X_CTRL_REFILL;
	            I2C_TxData(data, 2);
	            msleep(MMC3530X_DELAY_RST);
	            data[0] = MMC3530X_REG_CTRL;
	            data[1] = MMC3530X_CTRL_RESET;
	            I2C_TxData(data, 2);
	            msleep(1);
	            data[0] = MMC3530X_REG_CTRL;
	            data[1] = 0;
	...
	/* not check return value here, assume it always OK */
	I2C_TxData(data, 2);	
	msleep(MMC3530X_DELAY_TM);
	
#if READMD
	/* Read MD */
		data[0] = MMC3530X_REG_DS;
		I2C_RxData(data, 1);
		while (!(data[0] & 0x01)) {
			msleep(1);
			/* Read MD again*/
...
static int ECS_GetRawData(int data[3])
{
	...
	if(err !=0 )
	{
		printk(KERN_ERR "mmc3530X_IOC_TM failed\n");
		return -1;
	}

	// sensitivity 2048 count = 1 Guass = 100uT
	data[0] = (data[0] - MMC3530X_OFFSET_X) * 100 / MMC3530X_SENSITIVITY_X;
	data[1] = (data[1] - MMC3530X_OFFSET_X) * 100 / MMC3530X_SENSITIVITY_X;
	data[2] = (data[2] - MMC3530X_OFFSET_X) * 100 / MMC3530X_SENSITIVITY_X;
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_ReadChipInfo(char *buf, int bufsize)
{
	if((!buf)||(bufsize <= MMC3530X_BUFSIZE -1))
	{
		return -1;
	}
	if(!this_client)
...
static int mmc3530x_dev_init(struct i2c_client *client)
{
    u8 data[2];
    ...
    data[0] = MMC3530X_REG_BITS;
    data[1] = MMC3530X_BITS_SLOW_16;
    if (mmc3xxx_i2c_tx_data(  data, 2) < 0) {
        return MMC3530X_I2C_ERR;
    }
    msleep(MMC3530X_I2C_DELAY);
    ...
    return MMC3530X_SUCCESS;
}

/*----------------------------------------------------------------------------*/
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	char strbuf[MMC3530X_BUFSIZE];
	mmc3530x_ReadChipInfo(strbuf, MMC3530X_BUFSIZE);
...
{
	ssize_t res;
	struct mmc3530x_i2c_data *obj = i2c_get_clientdata(this_client);
	if(NULL == obj)
	{
		printk(KERN_ERR "mmc3530x_i2c_data is null!!\n");
		return 0;
	}	
...
/*----------------------------------------------------------------------------*/
static struct driver_attribute *mmc3530x_attr_list[] = {
    	&driver_attr_daemon,
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_create_attr(struct device_driver *driver) 
{
	int idx, err = 0;
	int num = (int)(sizeof(mmc3530x_attr_list)/sizeof(mmc3530x_attr_list[0]));
	if (driver == NULL)
	{
		return -EINVAL;
	}
	...
	{
		if((err = driver_create_file(driver, mmc3530x_attr_list[idx])))
		{            
			printk(KERN_ERR "driver_create_file (%s) = %d\n", mmc3530x_attr_list[idx]->attr.name, err);
			break;
		}
	}
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_delete_attr(struct device_driver *driver)
{
	int idx;
	int num = (int)(sizeof(mmc3530x_attr_list)/sizeof(mmc3530x_attr_list[0]));
	...
	for(idx = 0; idx < num; idx++)
	{
		driver_remove_file(driver, mmc3530x_attr_list[idx]);
	}
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_open(struct inode *inode, struct file *file)
{    
	struct mmc3530x_i2c_data *obj = i2c_get_clientdata(this_client);    
	int ret = -1;	
	...
	{
		MMCDBG("Open device node:mmc3530x\n");
	}
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_release(struct inode *inode, struct file *file)
{
	struct mmc3530x_i2c_data *obj = i2c_get_clientdata(this_client);
	atomic_dec(&dev_open_count);
	if(atomic_read(&obj->trace) & MMC_CTR_DEBUG)
	{
		MMCDBG("Release device node:mmc3530x\n");
	}	
	return 0;
}
/*----------------------------------------------------------------------------*/
//static int mmc3530x_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
	static long mmc3530x_unlocked_ioctl( struct file *file, unsigned int cmd,unsigned long arg)

{
	void __user *argp = (void __user *)arg;
		
	/* NOTE: In this function the size of "char" should be 1-byte. */
	char buff[MMC3530X_BUFSIZE];				/* for chip information */
	...
	switch (cmd)
	{
		...
		case MMC31XX_IOC_SET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				printk(KERN_ERR "mmc3530X_IOC_SET failed\n");
				return -EFAULT;
			}
			...
			data[1] = MMC3530X_CTRL_SET;
			if (I2C_TxData(data, 2) < 0) {
	                printk(KERN_ERR "mmc3530X_IOC_SET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3530X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	                printk(KERN_ERR "mmc3530X_IOC_SET failed3\n");
				return -EFAULT;
			}
			msleep(MMC3530X_DELAY_SET);
			break;
		...
		case MMC31XX_IOC_RESET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				printk(KERN_ERR "mmc3530X_IOC_RESET failed\n");
				return -EFAULT;
			}
			/* wait external capacitor charging done for next SET/RESET */
			msleep(MMC3530X_DELAY_RST);
			...
			data[1] = MMC3530X_CTRL_RESET;
			if (I2C_TxData(data, 2) < 0) {
	                printk(KERN_ERR "mmc3530X_IOC_RESET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3530X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	               printk(KERN_ERR "mmc3530X_IOC_RESET failed3\n");
				return -EFAULT;
			}
			msleep(1);
			break;
		...
		case MMC31XX_IOC_READ:
			...
			if(I2C_RxData(data, 6) < 0)
			{
				printk(KERN_ERR "mmc3530X_IOC_READ failed\n");
				return -EFAULT;
			}
			vec[0] = data[1] << 8 | data[0];
			vec[1] = data[3] << 8 | data[2];
		...
		//	vec[2] = 65536 - vec[2];
			...
			{
				printk(KERN_ERR "mmc3530X_IOC_READ: copy to user failed\n");
				return -EFAULT;
			}
			break;
		...
		case MMC31XX_IOC_READXYZ:
			...
			if(copy_to_user(argp, vec, sizeof(vec)))
			{
				printk(KERN_ERR "mmc3530X_IOC_READXYZ: copy to user failed\n");
				return -EFAULT;
			}
			break;	
		//++add read reg interface for mmc3530
		case MMC3530X_IOC_READ_REG:
			printk("MMC3530X_IOC_READ_REG\n");
			...
			break;
		case MMC3530X_IOC_WRITE_REG:
			printk(" MMC3530X_IOC_WRITE_REG\n");
			if (copy_from_user(&data, argp, sizeof(data)))
				return -EFAULT;
			if (I2C_TxData(data, 2) < 0)
				return -EFAULT;
		
			printk("MMC3530x_IOC_WRITE_REG '0x%0x' to register No. 0x%0x\n", data[0], data[1]);
		
			break;
		case MMC3530X_IOC_READ_REGS:
			printk(" MMC3530X_IOC_READ_REGS\n");
			if (copy_from_user(&data, argp, sizeof(data)))
				return -EFAULT;
			printk("MMC3530X_IOC_READ_REGS  %d registers from 0x%0x\n", data[1], data[0]);
			if (I2C_RxData(data, data[1]) < 0)
				return -EFAULT;
		
			printk("MMC3530X_IOC_READ_REGS  data: %x %x %x\n%x %x %x\n", data[0], data[1], data[2], data[3], data[4], data[5]);
		...
		case MSENSOR_IOCTL_READ_CHIPINFO:
			...
			mmc3530x_ReadChipInfo(buff, MMC3530X_BUFSIZE);
...
#ifdef CONFIG_COMPAT
static long mmc3530x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
		...
		switch(cmd)
		{
		   ...
		   case COMPAT_MMC3530X_IOC_READ_REG:
				ret = file->f_op->unlocked_ioctl(file, MMC3530X_IOC_READ_REG, (unsigned long)arg64);
				if(ret < 0)
				{
					printk(KERN_ERR "COMPAT_MMC3530x_IOC_READ_REG is failed!\n");
				}		
				break;
	
			case COMPAT_MMC3530X_IOC_WRITE_REG:
				ret = file->f_op->unlocked_ioctl(file, MMC3530X_IOC_WRITE_REG, (unsigned long)arg64);
				if(ret < 0)
				{
					printk(KERN_ERR "MMC3530X_IOC_WRITE_REG is failed!\n");
				}
	
				break; 
	
			case COMPAT_MMC3530X_IOC_READ_REGS:
				ret = file->f_op->unlocked_ioctl(file, MMC3530X_IOC_READ_REGS, (unsigned long)arg64);
				if(ret < 0)
				{
					printk(KERN_ERR "COMPAT_MMC3416X_IOC_READ_REGS is failed!\n");
				}	
				break; 
...
/*----------------------------------------------------------------------------*/
static struct file_operations mmc3530x_fops = {
	//.owner = THIS_MODULE,
	.open = mmc3530x_open,
	.release = mmc3530x_release,
	.unlocked_ioctl = mmc3530x_unlocked_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = mmc3530x_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
static struct miscdevice mmc3530x_device = {
    .minor = MISC_DYNAMIC_MINOR,
    .name = "msensor",
    .fops = &mmc3530x_fops,
};
/*----------------------------------------------------------------------------*/
int mmc3530x_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
int mmc3530x_orientation_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	if(atomic_read(&data->trace) & MMC_FUN_DEBUG)
	{
		MMCFUNC("mmc3530x_orientation_operate");
	}	
#endif
...
/*----------------------------------------------------------------------------*/
static int mmc3530x_suspend(struct i2c_client *client, pm_message_t msg) 
{
	struct mmc3530x_i2c_data *obj = i2c_get_clientdata(client)
	...
	{
		mmc3530x_power(obj->hw, 0);
	}
	return 0;
}
/*----------------------------------------------------------------------------*/
static int mmc3530x_resume(struct i2c_client *client)
{
	struct mmc3530x_i2c_data *obj = i2c_get_clientdata(client)


	mmc3530x_power(obj->hw, 1);
...
/*----------------------------------------------------------------------------*/
static void mmc3530x_early_suspend(struct early_suspend *h) 
{
	struct mmc3530x_i2c_data *obj = container_of(h, struct mmc3530x_i2c_data, early_drv);   
	...
	if(mmc3530x_SetPowerMode(obj->client, false))
	{
		printk("mmc3530x: write power control fail!!\n");
		return;
	}
	       
}
/*----------------------------------------------------------------------------*/
static void mmc3530x_late_resume(struct early_suspend *h)
{
	struct mmc3530x_i2c_data *obj = container_of(h, struct mmc3530x_i2c_data, early_drv);         
...
//static int mmc3530x_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info) 
//{    
//	strcpy(info->type, mmc3530X_DEV_NAME);
//	return 0;
//}

static int mmc3530x_m_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
...
//	    if ((err = mmc3530x_SetPowerMode(obj->client, ture)) < 0) {
//		    MAG_ERR("%s:mmc3530x_SetMode Error.\n", __FUNCTION__);
//		    return err;
//		}
	}
	else
	{
		...
		{
			atomic_set(&open_flag, 0);
//		    if ((err = mmc3530x_SetPowerMode(obj->client, false)) < 0) {
//		    MAG_ERR("%s:mmc3530x_SetMode Error.\n", __FUNCTION__);
//		    return err;
//			}
		}
...
static int mmc3530x_m_set_delay(u64 ns)
{
...
static int mmc3530x_m_open_report_data(int open)
{
	return 0;
}

static int mmc3530x_m_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int mmc3530x_o_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
...
static int mmc3530x_o_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= 10)
...
static int mmc3530x_o_open_report_data(int open)
{
	return 0;
}

static int mmc3530x_o_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
//	struct hwmsen_object sobj_m, sobj_o;
    ...
    MMCDBG("%s: ++++\n", __func__);
	if(!(data = kmalloc(sizeof(struct mmc3530x_i2c_data), GFP_KERNEL)))
	{
		err = -ENOMEM;
		goto exit;
	}
	memset(data, 0, sizeof(struct mmc3530x_i2c_data));

	data->hw = mmc3530x_get_cust_mag_hw();	
...
//modified for mmc3530x----changhua start
	/* Check connection */
	err = MMCECS_CheckDevice();
	if(err < 0)
	{
		MAG_ERR("mmc3530x probe: check device connect error\n");
		goto exit_init_failed;
	}

#if 1
	tmp[0] = MMC3530X_REG_CTRL;
	tmp[1] = MMC3530X_CTRL_REFILL;
	if (I2C_TxData(tmp, 2) < 0) {
	}
	msleep(MMC3530X_DELAY_SET);

	tmp[0] = MMC3530X_REG_CTRL;
	tmp[1] = MMC3530X_CTRL_SET;
	if (I2C_TxData(tmp, 2) < 0) {
	}
	msleep(1);
	tmp[0] = MMC3530X_REG_CTRL;
	...
	tmp[0] = MMC3530X_REG_BITS;
	tmp[1] = MMC3530X_BITS_SLOW_16;
	if (I2C_TxData(tmp, 2) < 0) {
	}
	msleep(MMC3530X_DELAY_TM);

	tmp[0] = MMC3530X_REG_CTRL;
	tmp[1] = MMC3530X_CTRL_TM;
	if (I2C_TxData(tmp, 2) < 0) {
	}
	msleep(MMC3530X_DELAY_TM);

#endif
	//modified for mmc3530x----changhua end


	/* Register sysfs attribute */
	if((err = mmc3530x_create_attr(&(mmc3530x_init_info.platform_diver_addr->driver))))
	{
	...
	if((err = misc_register(&mmc3530x_device)))
	{
		MAG_ERR( "mmc3530x_device register failed\n");
		goto exit_misc_device_register_failed;
	}    
/*
	sobj_m.self = data;
    sobj_m.polling = 1;
    sobj_m.sensor_operate = mmc3530x_operate;
	if((err = hwmsen_attach(ID_MAGNETIC, &sobj_m)))
*/
	//ctl.is_use_common_factory = false;
	ctl.m_enable = mmc3530x_m_enable;
	ctl.m_set_delay  = mmc3530x_m_set_delay;
	ctl.m_open_report_data = mmc3530x_m_open_report_data;
	ctl.o_enable = mmc3530x_o_enable;
	ctl.o_set_delay  = mmc3530x_o_set_delay;
	ctl.o_open_report_data = mmc3530x_o_open_report_data;
...
/*	sobj_o.self = data;
    sobj_o.polling = 1;
    sobj_o.sensor_operate = mmc3530x_orientation_operate;
	if((err = hwmsen_attach(ID_ORIENTATION, &sobj_o)))
*/

	mag_data.div_m = CONVERT_M_DIV;
	mag_data.div_o = CONVERT_O_DIV;
	mag_data.get_data_o = mmc3530x_o_get_data;
	mag_data.get_data_m = mmc3530x_m_get_data;
	...
	data->early_drv.suspend  = mmc3530x_early_suspend,
	data->early_drv.resume   = mmc3530x_late_resume,    
	register_early_suspend(&data->early_drv);
#endif

	MAG_ERR("%s: OK\n", __func__);
	mmc3530x_init_flag = 1;  
	return 0;
...
static int mmc3530x_i2c_remove(struct i2c_client *client)
{
	...
	if((err = mmc3530x_delete_attr(&(mmc3530x_init_info.platform_diver_addr->driver))))
	{
		MAG_ERR("mmc3530x_delete_attr fail: %d\n", err);
	}
	
	this_client = NULL;
	i2c_unregister_device(client);
	kfree(i2c_get_clientdata(client));	
	misc_deregister(&mmc3530x_device);    
	return 0;
}
...
static int mmc3530x_remove(void)
{
	struct mag_hw *hw = mmc3530x_get_cust_mag_hw();
 
	mmc3530x_power(hw, 0);    
	atomic_set(&dev_open_count, 0);  
	i2c_del_driver(&mmc3530x_i2c_driver);
	return 0;
}
...
static int mmc3530x_local_init(void)
{
	...
	atomic_set(&dev_open_count, 0);
	//mmc3530x_force[0] = hw->i2c_num;

	if(i2c_add_driver(&mmc3530x_i2c_driver))
	{
		MAG_ERR("mmc3530x add driver error\n");
		return -1;
	} 
	if(-1 == mmc3530x_init_flag)
	{
	   return -1;
	}
	return 0;
...
{
	struct mag_hw *hw = mmc3530x_get_cust_mag_hw();
	MAG_ERR("i2c_number=%d\n",hw->i2c_num);
	i2c_register_board_info(hw->i2c_num, &i2c_mmc3530x, 1);
...
/*	if(platform_driver_register(&mmc_sensor_driver))
	...
	mag_driver_add(&mmc3530x_init_info);
	return 0;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
module_init(mmc3530x_init);
module_exit(mmc3530x_exit);


MODULE_DESCRIPTION("mmc3530x compass driver");
MODULE_LICENSE("GPL");
