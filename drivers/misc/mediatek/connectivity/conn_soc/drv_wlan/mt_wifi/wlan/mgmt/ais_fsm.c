...
VOID aisFsmRunEventScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (ucSeqNumOfCompMsg != prAisFsmInfo->ucSeqNumOfScanReq) {
		...
		case AIS_STATE_ONLINE_SCAN:
			...
			//#ifndef VENDOR_EDIT
			//LiuMing@PSW.Connectivity.WLAN, 2016/01/27, Modify for [bug 760594: wifi disconnect when in "wifi analyser" screen]
			/*
			if (prAisBssInfo->ucDetectFailCount > 5) {
			*/
			//#else /* VENDOR_EDIT */
			if (prAisBssInfo->ucDetectFailCount > 10) {
			//#endif /* VENDOR_EDIT */
				prAisBssInfo->ucDetectFailCount = 0;
	...
	do {
		...
		if (prJoinCompMsg->ucSeqNum == prAisFsmInfo->ucSeqNumOfReqMsg) {
			...
			if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
				...
				prAisBssInfo->fgSecModeChanReassoc = FALSE;
#endif
				/* 1. Reset retry count */
				prAisFsmInfo->ucConnTrialCount = 0;
				...
				if (aisFsmStateInit_RetryJOIN(prAdapter, prStaRec) == FALSE) {
					...
					if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
						...
						eNextState = AIS_STATE_WAIT_FOR_NEXT_SCAN;
						 	...
						 	(prAisBssInfo->fgSecModeChanReassoc == TRUE)) {
						 /* abort connection trial */
						prAdapter->rWifiVar.rConnSettings.fgIsConnReqIssued = FALSE;
						prAdapter->rWifiVar.rConnSettings.eReConnectLevel = RECONNECT_LEVEL_MIN;

						if (prAisBssInfo->fgSecModeChanReassoc == TRUE)
							prAisBssInfo->u2DeauthReason = REASON_CODE_BSS_SECURITY_CHANGE;
						if ((prAisBssInfo->fgDisConnReassoc == TRUE) && prStaRec)
				...
				ENUM_PARAM_MEDIA_STATE_T eConnectionState, BOOLEAN fgDelayIndication)
{
	...
	if (!fgDelayIndication) {
		...
		else if (prAisBssInfo->fgSecModeChanReassoc)
			DBGLOG(AIS, INFO, "Reassoc the AP once beacause of AP's security mode is changed\n");
		else
#endif
		{
...
#if CFG_SUPPORT_AUTORN
			if ((prAisBssInfo->fgDisConnReassoc == FALSE) && (prAisBssInfo->fgSecModeChanReassoc == FALSE))
#endif
			{
...
/* For JB nl802.11 */
		if (prAisFsmInfo->prTargetBssDesc->ucJoinFailureCount < JOIN_MAX_RETRY_FAILURE_COUNT) {
			...
			u2StatusCode = STATUS_CODE_JOIN_TIMEOUT;
			kalIndicateStatusAndComplete(prAdapter->prGlueInfo,
										 WLAN_STATUS_CONNECT_INDICATION,
										 (PVOID)&u2StatusCode,
										 sizeof(UINT_16));
...
VOID aisBssSecurityChanged(P_ADAPTER_T prAdapter)
{
	...
	prAdapter->rWifiVar.rConnSettings.fgIsDisconnectedByNonRequest = FALSE;
	aisFsmStateAbort(prAdapter, DISCONNECT_REASON_CODE_RADIO_LOST, TRUE);
#else
