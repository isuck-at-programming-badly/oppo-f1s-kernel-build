...
VOID roamingFsmRunEventDiscovery(IN P_ADAPTER_T prAdapter, IN P_ROAMING_PARAM_T prParam)
{
	...
	//#ifndef VENDOR_EDIT
	//LiuMing@PSW.Connectivity.WLAN, 2016/02/05, Modify for [print roaming reason]
	/*
	DBGLOG(ROAMING, EVENT, "EVENT-ROAMING DISCOVERY: Current Time = %u Reason = %u\n",
		kalGetTimeTick(), prParam->u2Reason);
	*/
	//#else /* VENDOR_EDIT */
	printk("<wlan> EVENT-ROAMING DISCOVERY: Current Time = %u Reason = %u, PCPI=%d\n",
		kalGetTimeTick(), prParam->u2Reason,(UINT_8)(prParam->u2Data&0xff));
	//#endif /* VENDOR_EDIT */
