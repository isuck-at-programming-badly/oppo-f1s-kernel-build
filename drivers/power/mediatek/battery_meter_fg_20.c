...
//MoFei@EXP.BaseDrv.charge,2016/4/29 add for  non_standerd battery  compatibility of 15331
enum {
    Channel_12 = 12,
    Channel_13,
    Channel_14,
};
extern int get_board_volt(int times, int Channel);
...
static kal_int32 meter_fg_20_get_average_current(void)
{
	...
	if ((battery_meter_get_battery_current_sign() == true) && (BMT_status.bat_full == false)) //charging
		return -ret;
...
#ifdef VENDOR_EDIT_15331 
//MoFei@EXP.BaseDrv.charge,2016/5/19 add for  non_standerd battery  compatibility of 15331
	if (get_board_volt(5,Channel_12) >= 200) {//PVT : >200
