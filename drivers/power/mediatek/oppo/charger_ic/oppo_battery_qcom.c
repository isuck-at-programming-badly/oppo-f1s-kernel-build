...
static int smbchg_charging_disble(struct oppo_chg_chip *chip, bool flag)
{
...
static void smbchg_usb_update_online_work(struct work_struct *work)
{
	...
	online = user_enabled && chip->usb_present && !chip->very_weak_charger && !chip->other_sdp;
							...
							int current_ma)
{
	...
	if(usb_supply_type == POWER_SUPPLY_TYPE_USB){
		...
		pr_err("[%s]donot use pmic,use ap detect charger,usb_supply_type=%d--------\n",__func__,usb_supply_type);
	}
...
static int qpnp_get_battery_voltage(void)
{
	...
	rc = qpnp_vadc_read(the_chip->vadc_dev, VBAT_SNS, &adc_result);
	if (rc) {
							...
							int current_ma)
{
	...
	pr_err("[smbchg_set_thermal_limited]current_ma=%d,thermal_limit=%d\n",current_ma,chip->usb_tl_current_ma);
...
{
	struct oppo_chg_chip *chip = container_of(cdev,
			struct oppo_chg_chip, led_cdev);
	u8 reg;
	int rc;
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	struct oppo_chg_chip *chip = container_of(cdev, struct oppo_chg_chip,
			led_cdev);
	unsigned long blinking;
	ssize_t rc = -EINVAL;
...
static int smbchg_register_chg_led(struct oppo_chg_chip *chip)
{
	...
	rc = led_classdev_register(chip->dev, &chip->led_cdev);	//register a classdev ppp
	if (rc) {
...
static int opchg_hvdcp_enable(struct oppo_chg_chip *chip,bool enable)
{
	...
	rc = smbchg_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
...
static void handle_usb_removal(struct oppo_chg_chip *chip)
{
	...
	if (chip->force_aicl_rerun && !chip->very_weak_charger) {	//do nothing here ppp
		rc = smbchg_hw_aicl_rerun_en(chip, true);
		if (rc)
			pr_err("Error enabling AICL rerun rc= %d\n",
				rc);
	}
	pr_err("[handle_usb_removal]charger_type=%d----\n",chip->charger_type);
...
//	chip->charger_type = usb_supply_type;
	pr_err("[handle_usb_insertion]charger_type=%d-----------\n",chip->charger_type);
...
//				pr_err("[src_detect_handler]fast_charger started,return----\n");
//				return IRQ_HANDLED;
//	}
	...
	pr_err("%s:chip->usb_present = %d rt_sts = 0x%02x aicl = %d\n",__func__,chip->usb_present, reg,aicl_level);
	pr_smb(PR_STATUS,
...
static irqreturn_t src_detect_handler(int irq, void *_chip)
{
	...
	int rc;
	pr_err("%s: chip->usb_present = %d usb_present = %d src_detect = %d \n",__func__,
...
static int determine_initial_status(struct oppo_chg_chip *chip)
{
	...
	if((boot_mode == MSM_BOOT_MODE__RF) || (boot_mode == MSM_BOOT_MODE__WLAN)){
		...
		pr_err("[%s]:suspend_enable\n",__func__);
	}
	else {
		smbchg_usb_suspend_disable(chip);
		pr_err("[%s]:suspend_disable\n",__func__);
	}
	if (chip->usb_present) {
		pr_smb(PR_MISC, "setting usb psy dp=f dm=f\n");
		power_supply_set_dp_dm(chip->usb_psy,
...
static void smbchg_dump_reg_work(struct work_struct *work)
{
	...
	//schedule_delayed_work(&chip->dump_reg_work, msecs_to_jiffies(3000));
}
#endif
...
/*choose which pmic, return chip->wa_flags at 1110bit at rev > PMI8950 v1.0 by ppp*/
...
static int opchg_get_charger_type(void)
{
	...
	if(usb_supply_type == POWER_SUPPLY_TYPE_USB){
		...
		pr_err("[%s]donot usr pmic,use ap detect charger,usb_supply_type=%d--------\n",__func__,usb_supply_type);
	}
//	chip->charger_type = usb_supply_type;
//	chip->charger_name = usb_type_name;
...
{
	chip->bat_status = get_prop_batt_status(chip);
...
//   chip->temperature = opchg_get_prop_batt_temp(chip);
    chip->bat_instant_vol = get_prop_batt_voltage_now(chip);
    chip->charging_current = get_prop_batt_current_now(chip);//1000;
    chip->input_limit_flags = smbchg_is_input_current_limited(chip);
	chip->input_current_max = smbchg_get_aicl_level_ma(chip)*1000;
	chip->voltage_max = smbchg_float_voltage_get(chip);
	chip->charging_enable = smcghg_is_battchg_en(chip, REASON_BATTCHG_USER);
//    chip->charger_vol = opchg_get_prop_charger_voltage_now(chip);
    chip->capacity = get_prop_batt_capacity(chip);
	pr_err("%s middle--------",__func__);
	printk("ppp charger_type=%d,charger_name=%s,bat_vol=%d,,current_now=%d,input_limit_flag=%d,input_current_max=%d,vfloat=%d,capacity=%d,charging_state=%d,fast_current=%d\n",chip->charger_type,chip->charger_name,chip->bat_instant_vol,chip->charging_current,chip->input_limit_flags,
		chip->input_current_max,chip->voltage_max,chip->capacity,chip->bat_charging_state,chip->fastchg_current_ma);
	pr_err("pr_err ppp charger_type=%d,charger_name=%s,bat_vol=%d,,current_now=%d,input_limit_flag=%d,input_current_max=%d,vfloat=%d,capacity=%d,charging_state=%d,fast_current=%d\n",chip->charger_type,chip->charger_name,chip->bat_instant_vol,chip->charging_current,chip->input_limit_flags,
		chip->input_current_max,chip->voltage_max,chip->capacity,chip->bat_charging_state,chip->fastchg_current_ma);
	pr_err("%s end-------\n",__func__);
...
{
	int rc = 0;
	if(strcmp(chip->charger_name,"SDP") == 0)
...
static void opchg_update_thread(struct work_struct *work)
{
    ...
    /*update time 5s*/
    schedule_delayed_work(&chip->update_opchg_thread_work,msecs_to_jiffies(5000));
	pr_err("%s end\n", __func__);
}
#endif
#if 0
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->vfloat_adjust_work, smbchg_vfloat_adjust_work);//to adjust the vfloat.not seen yet ppp
	INIT_DELAYED_WORK(&chip->hvdcp_det_work, smbchg_hvdcp_det_work);//to detect the high volage charging,not seen yet ppp
//	INIT_DELAYED_WORK(&chip->dump_reg_work, smbchg_dump_reg_work);
	//schedule_delayed_work(&chip->dump_reg_work, msecs_to_jiffies(3000));
	...
	rc = smbchg_parse_peripherals(chip);//parse peripherals base ppp
	if (rc) {
		dev_err(chip->dev, "Error parsing DT peripherals: %d\n", rc);
		return rc;
	}

	rc = smbchg_check_chg_version(chip);//choose the chg_version(whether LITE) and choose which pmic, return chip->wa_flags ppp
	if (rc) {
		pr_err("Unable to check schg version rc=%d\n", rc);
		return rc;
	}

	rc = smb_parse_dt(chip);  //parse the orginal dts ppp
	if (rc < 0) {
...
//	#endif
	rc = smbchg_regulator_init(chip);	//init and register the otg device ppp
	if (rc) {
		dev_err(&spmi->dev,
	...
	rc = determine_initial_status(chip);	//init the status of the interrupt before request the interrupt and detect whether the usb or dc in or not  to respond ppp
	if (rc < 0) {
		dev_err(&spmi->dev,
	...
	rc = smbchg_request_irqs(chip);	//request the interrupt ppp
	if (rc < 0) {
	...
	dump_regs(chip);
	create_debugfs_entries(chip);	//net seen yet ppp
#ifdef PPPP
	...
	the_chip = chip;
	pr_err("[%s]SMBCHG successfully probe Charger version=%s Revision DIG:%d.%d ANA:%d.%d batt=%d dc=%d usb=%d\n",
			__func__,version_str[chip->schg_version],
			chip->revision[DIG_MAJOR], chip->revision[DIG_MINOR],
			chip->revision[ANA_MAJOR], chip->revision[ANA_MINOR],
