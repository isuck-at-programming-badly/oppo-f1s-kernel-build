...
static bool otg_is_enable = false;
...
int bq24196_enable_charging(struct oppo_chg_chip *chip)
{
	...
	if (vfloat_high) {
		chg_err(" vfloat_high true,return\n");
		return 0;
	}
	rc = bq24196_config_interface(chip, REG01_BQ24196_ADDRESS, 
...
int bq24196_disable_charging(struct oppo_chg_chip *chip, bool flag)
{
	int rc;
	...
	if (!flag) {
		return bq24196_suspend_charger(chip);
	} else {
...
int bq24196_hardware_init(struct oppo_chg_chip *chip)
{
	...
	if(get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) {
		...
		bq24196_disable_charging(chip, false);
#endif
