	...
	for(i = 0;i < READ_COUNT;i++){	//1508:448,1503:192
		...
		*/
		...
		if(addr == ((Stm8s_firmware_data[fw_line * 34 + 1] << 8) | Stm8s_firmware_data[fw_line * 34])){
			for(j = 0;j < 32;j++){
				if(data_buf[j] != Stm8s_firmware_data[fw_line * 34 + 2 + j]){
...
#ifdef CONFIG_OPPO_CHARGER_MTK
	for(i = 0;i < 32;i++) {
		if(data_buf[i] != Stm8s_firmware_data[sizeof(Stm8s_firmware_data) - 32 + i]) {
			chg_err("  fail,data_buf[%d]:0x%x != Stm8s_firmware_data[]:0x%x\n" ,
				i, data_buf[i], Stm8s_firmware_data[sizeof(Stm8s_firmware_data) - 32 + i]);
...
static int stm8s_fw_write(struct oppo_vooc_chip *chip,unsigned char *data_buf,unsigned int offset,unsigned int length)
{
...
#if 0	//st mcu don't swap low byte and high byte
		//because LSB is before MSB in buf,but stm8s receive MSB first
...
update_fw:
	...
	stm8s_fw_write(chip,Stm8s_firmware_data,0,sizeof(Stm8s_firmware_data) - 34);
	...
	rc = stm8s_fw_write(chip,Stm8s_firmware_data,sizeof(Stm8s_firmware_data) - 34,34);
	if(rc < 0){
		goto update_fw_err;
	}
...
//gpDMABuf_va = (u8 *)dma_alloc_coherent(NULL, GTP_DMA_MAX_TRANSACTION_LENGTH, &gpDMABuf_pa, GFP_KERNEL);
	...
	chip->firmware_data = Stm8s_firmware_data;
	chip->fw_data_count = sizeof(Stm8s_firmware_data);
	chip->fw_data_version = Stm8s_firmware_data[chip->fw_data_count - 4];
