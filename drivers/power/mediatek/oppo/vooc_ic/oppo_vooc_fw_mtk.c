...
int oppo_vooc_gpio_dt_init(struct oppo_vooc_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,vooc_data_irq", &chip->vooc_gpio.data_irq);
    if (rc) {
...
void oppo_vooc_eint_register(struct oppo_vooc_chip *chip)
{
	...
	mt_set_gpio_mode(chip->vooc_gpio.data_gpio, GPIO_MODE_00);
	mt_set_gpio_dir(chip->vooc_gpio.data_gpio,GPIO_DIR_IN);
	mt_set_gpio_pull_enable(chip->vooc_gpio.data_gpio, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(chip->vooc_gpio.data_gpio, GPIO_PULL_UP);
    ...
    //mt_eint_registration(chip->vooc_gpio.data_irq, IRQF_TRIGGER_RISING, irq_rx_handler, 0);
	//mt_eint_unmask(chip->vooc_gpio.data_irq);
}
