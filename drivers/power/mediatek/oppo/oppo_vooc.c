...
static void oppo_vooc_fastchg_func(struct work_struct *work)
{
	...
	int volt = 0, temp = 0, soc = 0, current_now = 0, chg_vol = 0;
	...
	if(data == VOOC_NOTIFY_FAST_PRESENT) 
	{
		...
		mod_timer(&chip->watchdog, jiffies+msecs_to_jiffies(15000));
		if(!isnot_power_on){
			isnot_power_on = true;
			ret_info = 0x1;
		} else {
	...
	else if(data == VOOC_NOTIFY_ALLOW_READING_IIC) 
	{
		...
		//don't read
		chip->allow_reading = false;
		vooc_xlog_printk(CHG_LOG_CRTI, " volt:%d,temp:%d,soc:%d,current_now:%d\n",volt,temp, soc,current_now);	
		mod_timer(&chip->watchdog, jiffies+msecs_to_jiffies(10000));
		ret_info = 0x2;
	}
...
void oppo_vooc_print_log(void)
{
	...
	vooc_xlog_printk(CHG_LOG_CRTI, "[VENDOR VOOC] fastchg_allow = %d, started = %d, dummy_started = %d, to_normal = %d, to_warm = %d, btb = %d\n", 
