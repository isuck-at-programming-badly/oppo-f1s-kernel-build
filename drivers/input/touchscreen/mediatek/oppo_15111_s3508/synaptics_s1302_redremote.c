...
#define DEVICE_CLASS_NAME "rmidev_s1302"
#define DEV_NUMBER 1
...
static int remote_rmi4_i2c_read(unsigned short addr, unsigned char *rxbuf, unsigned short len);
static int remote_rmi4_i2c_write(unsigned short addr, unsigned char *txbuf, unsigned short len);
...
int remote_rmi4_i2c_read(unsigned short addr, unsigned char *rxbuf, unsigned short len)
{
	int ret;
	s32 retry = 0;
	u8 buffer[1];
	struct i2c_client* client = remote_rmi4_get_i2c_client();

	struct input_dev *input_dev = remote_rmi4_get_input();
	gprDMABuf_va = (u8 *)dma_alloc_coherent(&input_dev->dev, 255, &gprDMABuf_pa, GFP_KERNEL);
	if (!gprDMABuf_va)
		dev_err(device_ptr,"%s: [Error] Allocate DMA I2C buffer failed!\n",__func__);
	...
	printk("srd dma i2c read: 0x%x, %d bytes(s)", addr, len);
	ret = remote_rmit_set_page(addr);
	if (ret < 0)
	{
		goto exit;
	}
	...
	printk("Dma I2C Read Error: 0x%04X, %d byte(s), err-code: %d", addr, len, ret);
...
int remote_rmi4_i2c_write(unsigned short addr, unsigned char *txbuf, unsigned short len)
{
	int ret;
	s32 retry = 0;
	...
	gpwDMABuf_va = (u8 *)dma_alloc_coherent(&input_dev->dev, 255, &gpwDMABuf_pa, GFP_KERNEL);
	if (!gpwDMABuf_va)
		dev_err(device_ptr,"%s: [Error] Allocate DMA I2C buffer failed!\n",__func__);
	buf_va = gpwDMABuf_va;
	struct i2c_msg msg =
	...
	buf_va[0] = (u8)(addr & 0xFF);

	memcpy(buf_va+1, txbuf, len);
	for (retry = 0; retry < 5; ++retry)
	{
	...
	printk("Dma I2C Write Error: 0x%04X, %d byte(s), err-code: %d", addr, len, ret);
...
int register_remote_device_s1302(struct remotepanel_data *pdata)
{
	...
	dev_t dev_no;
	struct rmidev_data *dev_data = NULL;
	//gmq modify  to global value  struct rmidev_data *dev_data = NULL;
