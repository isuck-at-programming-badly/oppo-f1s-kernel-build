...
#define TPD_DEVICE "mtk-tpd-s3508"
...
#endif
#ifdef DMA_SUPPORT
int i2c_dma_write(const struct i2c_client *client, u8 addr, s32 len, u8 *txbuf);
int i2c_dma_read(const struct i2c_client *client, u8 addr, s32 len, u8 *txbuf);
struct st_i2c_msgs
{
	struct i2c_msg *msg;
	int count;
...
s32 i2c_smbus_write_byte_data_s3508(const struct i2c_client *client, u8 command, u8 value)
{
	int ret=0;
...
s32 i2c_smbus_read_byte_data_s3508(const struct i2c_client *client, u8 command)
{
	int ret=0;
...
s32 i2c_smbus_write_word_data_s3508(const struct i2c_client *client, u8 command,u16 value)
{
	int ret=0;
...
void write_back_reg_status(struct synaptics_ts_data *ts)
{
	int ret;
...
static int synaptics_soft_reset(struct synaptics_ts_data *ts)
{
	...
	if(0 == limit_enable)
	{
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client, F51_CUSTOM_CTRL_BASE + 0x1D, 0x4A);
	}
	else if(1 == limit_enable)
	{
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client, F51_CUSTOM_CTRL_BASE + 0x1D, 0x4B);
	}
...
static int synaptics_read_register_map(struct synaptics_ts_data *ts)
{
	...
	F51_CUSTOM_DATA05 = F51_CUSTOM_DATA_BASE;
...
//mingqiang.guo for charge pulg in ,open tp Finger Amplitude Thre and Finger Dbounce ,avoid charge disturb
int charge_plug_tp_avoid_distrub(int enable,int is_fast_charge)
{
...
int i2c_dma_read(const struct i2c_client *client, u8 addr, s32 len, u8 *rxbuf)
{
	int ret;
	s32 retry = 0;
	u8 buffer[1];
...
int i2c_read_bytes_dma(struct i2c_client *client, u16 addr, u8 *rxbuf, s32 len)
{
	s32 left = len;
	s32 read_len = 0;
	u8 *rd_buf = rxbuf;
...
static void synaptics_get_coordinate_point(struct synaptics_ts_data *ts)
{
	...
	ret = synaptics_rmi4_set_page(ts->client, 0xff, 0x4); 
	ret = i2c_dma_read(ts->client, F51_CUSTOM_DATA05, 8, &(coordinate_buf[0])); // 14045 use F51_CUSTOM_DATA05
	ret = i2c_dma_read(ts->client, F51_CUSTOM_DATA05 + 8, 8, &(coordinate_buf[8]));
	ret = i2c_dma_read(ts->client, F51_CUSTOM_DATA05 + 16, 8, &(coordinate_buf[16])); 
	ret = i2c_dma_read(ts->client, F51_CUSTOM_DATA05 + 24, 1, &(coordinate_buf[24]));
	for(i = 0; i< 23; i += 2){
...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	ret = synaptics_rmi4_set_page(ts->client, 0xff, 0x4); 
	regswipe = i2c_smbus_read_byte_data_s3508(ts->client, F51_CUSTOM_DATA05+0x18); //14045 has no WakeUpGesture Swipe Direction 
			...
			if(((obj_attention&0x03FF) >> i )&0x01   &&  ( points.status != 0 )  ) //mingqiang.guo modify for reslove dirty touch point
			{
				...
				else
				{
					if( touch_major_report_num == 1)
					{
						 ...
						 input_sync(ts->input_dev);
						 input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, (points.raw_x+points.raw_y)/2 + 1);		//add 1,ABS_MT_WIDTH_MAJOR change, in order to get action MOVE
...
TEST_WITH_CBC_s3508:
	...
	ret = i2c_smbus_write_byte_data_s3508(ts_g->client, F54_ANALOG_CONTROL_BASE+20, 0x01);
	ret = i2c_smbus_read_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+23);
	tmp_old = ret&0xff;

	if(enable_cbc){
		TPD_DEBUG("ret = %x ,tmp_old =%x ,tmp_new = %x\n",ret,tmp_old,(tmp_old | 0x10));
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+23,(tmp_old | 0x10));
		ret = i2c_smbus_write_word_data_s3508(ts_g->client,F54_ANALOG_COMMAND_BASE,0x04);
		checkCMD();
		ret = i2c_smbus_read_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+27);
		tmp_new = ret | 0x20;
		i2c_smbus_write_byte_data_s3508(ts_g->client, F54_ANALOG_CONTROL_BASE+27, tmp_new);
		ret = i2c_smbus_write_word_data_s3508(ts_g->client, F54_ANALOG_COMMAND_BASE, 0x04);
		TPD_DEBUG("Test open cbc\n");
		baseline_data_test = (uint16_t *)(fw->data + ph->array_limitcbc_offset);	
	}else{
		TPD_DEBUG("ret = %x ,tmp_old =%x ,tmp_new = %x\n",ret,tmp_old,(tmp_old & 0xef));
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+23,(tmp_old & 0xef));	
		ret = i2c_smbus_write_word_data_s3508(ts_g->client, F54_ANALOG_COMMAND_BASE, 0x04);
		ret = i2c_smbus_read_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+27);
		tmp_new = ret & 0xdf;
		i2c_smbus_write_byte_data_s3508(ts_g->client, F54_ANALOG_CONTROL_BASE+27, tmp_new);
	...
	for(x = 0;x < TX_NUM; x++)  
	{
		...
		//mingqiang.guo del for 15019 no virtual key 	if(  ( (x < TX_NUM-3) && (y < RX_NUM-1 ) ) 
		//	||  ( (x > TX_NUM-4) && (y == RX_NUM-1 ) )   )// x-3 ,y-1  can not judge key line, 3 horizontal line and 1 ordinate line realize virtual key  
			{ 
...
static ssize_t tp_baseline_show_with_cbc(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	ret = i2c_smbus_write_byte_data_s3508(ts_g->client,F54_ANALOG_COMMAND_BASE,0X04);//force F54_ANALOG_CMD00
	checkCMD();
	TPD_DEBUG("forbid Forbid NoiseMitigation oK\n");
...
/*	if (driver_create_file(&tpd_i2c_driver.driver, &driver_attr_oppo_tp_baseline_image))
	...
	/*prEntry_temp = proc_create("oppo_tp_plam_threshold", 0644, prEntry_tp,&proc_plam_threshold);
	if(prEntry_temp == NULL)
	{            
		ret = -ENOMEM;	   
		TPDTM_DMESG(KERN_INFO"init_synaptics_proc: Couldn't create proc entry\n");
...
//mingqiang.guo for charge pulg in ,open tp Finger Amplitude Thre and Finger Dbounce ,avoid charge disturb
	...
	strcpy(ts->fw_name,"tp/15111_FW_S3508_Samsung.img");	
	strcpy(ts->test_limit_name,"tp/15111_Limit_Samsung.img");
...
struct delayed_work	syp_s3508_work;
...
static int __init tpd_driver_init(void) {
	...
	if(! is_project(OPPO_16031) )
	{
		TPD_ERR("tpd_IC is 3508!\n");	
	}
	else
	{
		TPD_ERR("tpd_IC is 3320!Synaptics_s3508 exit\n");	
		return 0;
	}
