...
static ssize_t tp_reg_operate_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	ret = sprintf(page, "read reg=0x%x is 0x%x, sleep = 0x%x, interrupt = 0x%x\n", tp_reg_address,buf[2], sleep, interrupt);
	ret = simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));
	return ret;
}
...
static int synaptics_enable_interrupt(struct synaptics_ts_data *ts, int enable)
{
	...
	//TPD_ERR("synaptics_enable_interrupt called!\n");
	ret = synaptics_rmi4_set_page(ts->client, 0xff, 0x0); 
	if(ret < 0) {
...
TEST_WITH_CBC_s3508:
	...
	if(enable_cbc){
		...
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+0x29,(tmp_old | 0x20));
		...
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client,F54_ANALOG_CONTROL_BASE+0x29,(tmp_old & 0xdf));	
		ret = i2c_smbus_write_byte_data_s3508(ts_g->client, F54_ANALOG_CONTROL_BASE+7, 0x01);// Forbid NoiseMitigation
	...
	for(x = 0;x < TX_NUM; x++)  
	{
		for(y = 0; y < RX_NUM; y++) 
		{
			...
			TPD_ERR("x = %d y = %d baseline_data is %d\n", x, y, baseline_data);
	...
	for(x = 0;x < 7; x++)
	{
		TPD_ERR("Check rx-rx :buf[%d]=%d", x, buffer[x]);
		if(buffer[x]){
			error_count++;
			goto END;	
		}
