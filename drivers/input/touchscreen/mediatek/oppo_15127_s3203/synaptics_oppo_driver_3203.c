...
static ssize_t cap_vk_show(struct kobject *kobj, struct kobj_attribute *attr,
	...
	return sprintf(buf,
			__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":118:1370:100:80"
			":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":360:1370:100:80"
			":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":600:1370:100:80"		 
			"\n");
}
...
s32 i2c_smbus_write_byte_data_s3203(const struct i2c_client *client, u8 command, u8 value)
{
	int ret=0;
...
s32 i2c_smbus_read_byte_data_s3203(const struct i2c_client *client, u8 command)
{
	int ret=0;
...
s32 i2c_smbus_write_word_data_s3203(const struct i2c_client *client, u8 command,u16 value)
{
	int ret=0;
...
static int synaptics_enable_interrupt_for_gesture(struct synaptics_ts_data *ts, int enable)
{
	...
	unsigned char val[4];
	/* page select = 0x0 */
	//F12_2D_CTRL20 = F11_2D_CTRL_BASE + 0x07;
	//F12_2D_CTRL27 = F11_2D_CTRL_BASE + 0x0c;
...
int tp_reg_address;
static int tp_reg_operate_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	int ret = 0;
	char page[PAGESIZE];
	char buf[50];
...
static int tp_reg_operate_write_func(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	int ret = 0 ;
	char page[PAGESIZE];
	char buf[50];
...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	unsigned char reportbuf[3];
	int F12_2D_CTRL20;
	static F11_2D_DATA39 = 0x004C;
	static F11_2D_DATA38 = 0x004B;
	int status_int;
...
static ssize_t tp_show(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	//ret0 = i2c_smbus_read_byte_data_s3203(ts_g->client, 0x14);
	//if(ret0 < 0)
	//TPDTM_DMESG("tp_show read i2c err\n");
	//update_lcd();
...
static ssize_t store_tp(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	int tmp = 0;
	struct synaptics_ts_data *ts = ts_g; 
	int i;
...
static int synaptics_fw_check(struct synaptics_ts_data *ts )
{
	...
	/* 	ret = i2c_smbus_read_byte_data_s3203(ts->client, F11_2D_CTRL06);
	if(ret > 0)
		max_x_ic = ret&0xffff; 
	
	ret = i2c_smbus_read_byte_data_s3203(ts->client, F11_2D_CTRL08);
	if(ret > 0)
		max_y_ic = ret&0xffff; */
...
static int synapitcs_ts_update(struct i2c_client *client, const uint8_t *data, uint32_t data_len ,bool force)
{
	...
	for(j=0; j<firmware; j++) 
	{
		...
		i2c_dma_write(client,SynaF34Reflash_BlockData,16,&Firmware_Data[j*16]); 
	...
	for(j=0;j<configuration;j++)
	{
		...
		i2c_dma_write(client,SynaF34Reflash_BlockNum,2,buf);
		//b) write data
		i2c_dma_write(client,SynaF34Reflash_BlockData,16,&Config_Data[j*16]);
		//c) issue write
...
//static DRIVER_ATTR(baseline_test, 0664, synaptics_rmi4_baseline_show_s3508, tp_test_store);
//static DRIVER_ATTR(oppo_tp_baseline_image, 0664, tp_baseline_show, tp_delta_store);
//static DRIVER_ATTR(oppo_tp_baseline_image_with_cbc, 0664, tp_baseline_show_with_cbc, tp_delta_store);
//static DRIVER_ATTR(oppo_tp_debug, 0664, tp_show, store_tp);
//static DRIVER_ATTR(oppo_tp_delta_data, 0664, tp_delta_show, NULL);
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//static int F01_RMI_CTRL00_TEMP=0x0051;
	...
	/* if((boot_mode == META_BOOT)  || (boot_mode == FACTORY_BOOT))
	{
		...
		ret = i2c_smbus_write_byte_data(ts->client, F01_RMI_CTRL00_TEMP, 0x01); 
		if( ret < 0 )
			TPD_ERR("....write F01_RMI_CTRL00_TEMP error....\n");
		ret = i2c_smbus_write_byte_data(client, 0xff, 0x00); 
		ret = i2c_smbus_read_byte_data(client, F01_RMI_CTRL00_TEMP); 
		TPDTM_DMESG("F01_RMI_CTRL00_TEMP=0x%x,ret= 0x%x \n",F01_RMI_CTRL00_TEMP,ret);
		if( ret < 0 ){
...
//end
	...
	/* mt_set_gpio_mode(GPIO_TP_ID1, 0);  
  	mt_set_gpio_dir(GPIO_TP_ID1, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_TP_ID1, 0);

	mt_set_gpio_mode(GPIO_TP_ID2, 0);  
  	mt_set_gpio_dir(GPIO_TP_ID2, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_TP_ID2, 0); */
