...
#if GTP_HEADER_FW_UPDATE
	GTP_ERROR("Start auto update thread_1\n");
	gt1x_update_firmware(NULL);
#else
	int ret;
	char *filename;
	u8 config[GTP_CONFIG_MAX_LENGTH] = { 0 };
	GTP_ERROR("Start auto update thread_2\n");
...
int gt1x_update_firmware(char *filename)
{
	...
	if (ret) {
		GTP_ERROR("....error::Update_3....\n!");
		goto gt1x_update_exit;
	}
	update_info.progress = 3;
	GTP_ERROR("....update_info.progress = 3....\n!");
	...
	GTP_ERROR("....update_info.progress = 6....\n!");
	gt1x_reset_guitar();
	p = gt1x_get_fw_data(update_info.firmware->subsystem[0].offset, update_info.firmware->subsystem[0].length);
	if (p == NULL) {
		GTP_ERROR("get isp fail");
		GTP_INFO("....error::Update_6....\n!");
		ret = ERROR_FW;
		goto gt1x_update_exit;
	}
	update_info.progress = 7;
	GTP_ERROR("....update_info.progress = 7....\n!");

	ret = gt1x_run_ss51_isp(p, update_info.firmware->subsystem[0].length);
	if (ret) {
		GTP_ERROR("run isp fail");
		GTP_INFO("....error::Update_7....\n!");
		goto gt1x_update_exit;
	}
...
int gt1x_update_judge(void)
{
	...
	GTP_INFO("GTP::mask_id=0x%x\n",fw_ver_info.mask_id);
	GTP_INFO("GTP::patch_id=0x%x\n",fw_ver_info.patch_id);
	GTP_INFO("GTP::product_id=0x%x\n",*fw_ver_info.product_id);
	...
	if ((fw_ver_info.patch_id & 0xFFFF) == (ver_info.patch_id & 0xFFFF)) {
		...
		GTP_ERROR("fw_ver_info=ver_info!don't need update!\n");
		return ERROR_CHECK;
	}else{
		GTP_ERROR("fw_ver_info=0x%x\n",fw_ver_info.patch_id);
		GTP_ERROR("ver_info=0x%x\n",ver_info.patch_id);
		GTP_ERROR("fw_ver_info!=ver_info!need update!\n");
		return 0;
	}
	return 0;
}
