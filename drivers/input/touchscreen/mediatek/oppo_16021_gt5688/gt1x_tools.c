...
static s32 gt1x_tool_write(struct file *filp, const char __user * buff, size_t len, loff_t * data)
{
	...
	if(tpd_halt)
  	{
  		  GTP_INFO("tpd suspend\n");
  		  if(gt1x_rawdiff_mode==true)
...
static s32 gt1x_tool_read(struct file *filp, char __user * buffer, size_t count, loff_t * ppos)
{
   ...
   if(*ppos)
	...
	if (cmd_head.wr % 2) {
		...
		page[0] = update_info.progress >> 8;
		page[1] = update_info.progress & 0xff;
		page[2] = update_info.max_progress >> 8;
		page[3] = update_info.max_progress & 0xff;
		//*ppos += 4;
		simple_read_from_buffer(buffer, count, ppos, page, 4);
		//ret = copy_to_user(buffer,page,strlen(page));
		return 4;
		...
		copy_to_user(buffer, GTP_DRIVER_VERSION, tmp_len);
		buffer[tmp_len] = 0;
		*ppos += tmp_len + 1;
		return (tmp_len + 1);
	}
