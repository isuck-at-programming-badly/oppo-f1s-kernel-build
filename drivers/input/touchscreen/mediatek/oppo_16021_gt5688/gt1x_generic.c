...
//Chenggang.Li@BSP add for 16021 2016.05.11
/**********************start***************/
#define SENSOR_NUM   15
#define DRIVER_NUM   27
#define GTP_REG_RAWDATA  0xB798
#define GTP_REG_DIFFDATA 0xAF58
#define GTP_REG_BASEDATA 0xB378
const int gtp_data_addrs[] = {GTP_REG_RAWDATA, GTP_REG_DIFFDATA, GTP_REG_BASEDATA};
const int gtp_len_of_per_sensor[] = {SENSOR_NUM *2 , SENSOR_NUM, SENSOR_NUM * 2};
...
static int tp_devices_check_read_func(struct file *file, char __user * page, size_t size, loff_t * ppos);
...
s32 gt1x_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
	...
	if (len > 16) {
		GTP_ERROR("i2c_read_dbl_check length %d is too long, do not beyond %ld", len, sizeof(buf));
		return ERROR;
	}
...
//Chenggang.Li@bsp add for 16021 2016.05.11
	...
	sprintf(TP_FW_ID,"0x%x",((cfg_len&0x000000ff)|firmware_id));
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	//GTP_INFO("IC VERSION: GT%s_%04X(Patch)_%04X(Mask)_%02X(SensorID)", product_id, patch_id, mask_id >> 8, sensor_id);//chenggang.li@modified for oppo patch_id >> 8
	...
	if (ver_info != NULL) {
        ...
        #ifdef VENDOR_EDIT
        //xiongxing@EXP.BaseDrv.TP, add for export project 16321
        ver_info->sensor_id = sensor_id + 3;
        #endif
...
s32 gt1x_get_chip_type(void)
{
	...
	if (ret) {
		...
		GTP_ERROR("goodix-------------goodix-------------I2c communication error.");
		return -1;
	}
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			if (!ret) {
				...
				/* i2c test succeed, init externl watchdog */
#if GTP_ESD_PROTECT
				if (!(ret = gt1x_init_ext_watchdog_gesture())) {
					break;
				}
#else
				break;
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				input_x = GTP_WARP_X(gt1x_abs_x_max, input_x);
				input_y = GTP_WARP_Y(gt1x_abs_y_max, input_y);
				//touch down 100 time , printk one coordinate log  
				/* if( touch_down_time == 100) 
				{
					touch_down_time = 0;
					GTP_INFO("touch Down(%d) (%d, %d) [%d]", id, input_x, input_y, input_w);
				}
				else
				{
					touch_down_time++;
					GTP_DEBUG("touch Down(%d) (%d, %d) [%d]", id, input_x, input_y, input_w);
				} */
...
static int init_gt1x_glove_proc(void)
{
	int ret = 0;
	if(prEntry_tp==NULL)
	{
		GTP_INFO("+++++++++++++++ prEntry_tp==NULL ,can not creat glove_mode_check\n");
		return -1;
	}
	prEntry_dtap = proc_create("glove_mode_check", 0777, prEntry_tp, &gt1x_devices_check);
	
	if (prEntry_dtap == NULL) {
		GTP_ERROR("create_proc_entry DEVICES_CHECK_GOODIX  FAILED!\n");
		return -1;
	}
	GTP_INFO("create_proc_entry DEVICES_CHECK_GOODIX SUCCESS.\n");
	return 0;
