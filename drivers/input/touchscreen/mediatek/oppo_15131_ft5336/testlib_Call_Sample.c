...
//保存测试数据到SD卡 etc.
#define FTXXXX_INI_FILEPATH_TWO "/sdcard/"
static int ftxxxx_SaveTestData(char *file_name, char *data_buf, int iLen)
{
	...
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", FTXXXX_INI_FILEPATH_TWO, file_name);

	old_fs = get_fs();		
	set_fs(KERNEL_DS);
	fd = sys_open(filepath, O_RDWR | O_CREAT | O_TRUNC, 0);
	pr_err("fd::'%d'\n", fd);
	if (fd < 0) { 
		pr_err("filepath::'%s'\n", filepath);
		pr_err("!!!!!!!!!!Open filepath failed!!!!!!!!!\n");			
		set_fs(old_fs);		
	}
