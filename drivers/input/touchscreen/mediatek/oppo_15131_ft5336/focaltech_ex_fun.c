...
struct i2c_client *ft_client =NULL;
...
static unsigned char CTPM_FW[] = {
	//#include "15066001_truly.i"
	#include "15066101_ofilm.i"
};
...
static unsigned char CTPM_FW_BIEL[] = {
	#include "15066101_biel.i"
};
static unsigned char CTPM_FW_TRULY_NT[] = {
	#include "15066001_truly_nt.i"
};
...
static int tp_choose =0; 
void choose_tp_fw(int tp_dev)
{	
	//return;
	//int len =strlen(CTPM_FW_TRULY)+1;
	//printk("huqiao_len=%d\n",len);
	if(tp_dev == 0)
		tp_choose = 0;
	if(tp_dev == 1)
	...
	if(tp_dev == 3)
		tp_choose = 3;
	printk("mtk_tpd[FTS] tp_choose = 0x%x\n", tp_choose);
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	if(tp_choose == 1)
	{
		...
		dev_err(&client->dev,"chenggang_TRULY_fw_len=%d\n",fw_len);
		if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) 
				...
				||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
		{
			...
			if ((CTPM_FW_TRULY[fw_len - 8] ^ CTPM_FW_TRULY[fw_len - 6]) == 0xFF
				&& (CTPM_FW_TRULY[fw_len - 7] ^ CTPM_FW_TRULY[fw_len - 5]) == 0xFF
				&& (CTPM_FW_TRULY[fw_len - 3] ^ CTPM_FW_TRULY[fw_len - 4]) == 0xFF) 
			{
				/*FW upgrade */
				...
				{
					i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY));
				}else if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12)
						||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14))
				{
					i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY));
				}
				else if ((fts_updateinfo_curr.CHIP_ID==0x06))
				{
					i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY));
				}
				if (i_ret != 0)
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		...
		pbt_buf = CTPM_FW_TRULY;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		dev_err(&client->dev, "%s:.....chenggang...CHIP_ID=0x54\n", __func__);
		...
		pbt_buf = CTPM_FW_TRULY;
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
	}	
...
if(tp_choose == 2)
{
				...
				||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
		{
			...
			if ((CTPM_FW_BIEL[fw_len - 8] ^ CTPM_FW_BIEL[fw_len - 6]) == 0xFF
				&& (CTPM_FW_BIEL[fw_len - 7] ^ CTPM_FW_BIEL[fw_len - 5]) == 0xFF
				&& (CTPM_FW_BIEL[fw_len - 3] ^ CTPM_FW_BIEL[fw_len - 4]) == 0xFF) 
			{
				/*FW upgrade */
				...
				if ((fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x08) ||(fts_updateinfo_curr.CHIP_ID==0x0a))
				{
					i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_BIEL));
				}else if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12)
						||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14))
				{
					i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_BIEL));
				}
				else if ((fts_updateinfo_curr.CHIP_ID==0x06))
				{
					i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_BIEL));
				}
				if (i_ret != 0)
					dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
				else if(fts_updateinfo_curr.AUTO_CLB==AUTO_CLB_NEED)
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		...
		pbt_buf = CTPM_FW_BIEL;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_BIEL));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		pbt_buf = CTPM_FW_BIEL;
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_BIEL));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
	}	
...
if(tp_choose == 0){
		...
		printk("chenggang_ofilm_fw_len=%d\n",fw_len);
		if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) 
			||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)
			||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) 
			||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
		...
		if ((CTPM_FW_OFILM[fw_len - 8] ^ CTPM_FW_OFILM[fw_len - 6]) == 0xFF
			&& (CTPM_FW_OFILM[fw_len - 7] ^ CTPM_FW_OFILM[fw_len - 5]) == 0xFF
			&& (CTPM_FW_OFILM[fw_len - 3] ^ CTPM_FW_OFILM[fw_len - 4]) == 0xFF) 
		{
			/*FW upgrade */
			...
			if ((fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x08) ||(fts_updateinfo_curr.CHIP_ID==0x0a))
			{
				i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM));
			}else if ((fts_updateinfo_curr.CHIP_ID==0x11)||(fts_updateinfo_curr.CHIP_ID==0x12) 
					||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14))
			{
				i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM));
			}
			else if ((fts_updateinfo_curr.CHIP_ID==0x06))
			{
				i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM));
			}
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		...
		pbt_buf = CTPM_FW_OFILM;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		pbt_buf = CTPM_FW_OFILM;
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
	}	
	...
	{
		msleep(5);
		fw_len = sizeof(CTPM_FW_TRULY_NT);
		dev_err(&client->dev,"chenggang_TRULY_fw_len=%d\n",fw_len);
		if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) 
				||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)
				||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) 
				||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
		{
			...
			if ((CTPM_FW_TRULY_NT[fw_len - 8] ^ CTPM_FW_TRULY_NT[fw_len - 6]) == 0xFF
				&& (CTPM_FW_TRULY_NT[fw_len - 7] ^ CTPM_FW_TRULY_NT[fw_len - 5]) == 0xFF
				&& (CTPM_FW_TRULY_NT[fw_len - 3] ^ CTPM_FW_TRULY_NT[fw_len - 4]) == 0xFF) 
			{
				/*FW upgrade */
				pbt_buf = CTPM_FW_TRULY_NT;
				/*call the upgrade function */
				if ((fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x08) ||(fts_updateinfo_curr.CHIP_ID==0x0a))
				{
					i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY_NT));
				}else if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12)
						||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14))
				{
					i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY_NT));
				}
				else if ((fts_updateinfo_curr.CHIP_ID==0x06))
				{
					i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY_NT));
				}
				if (i_ret != 0)
					dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
				else if(fts_updateinfo_curr.AUTO_CLB==AUTO_CLB_NEED)
		...
		else if ((fts_updateinfo_curr.CHIP_ID==0x36))
		{
			...
			pbt_buf = CTPM_FW_TRULY_NT;
			i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY_NT));
			if (i_ret != 0)
				dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
		}
		else if ((fts_updateinfo_curr.CHIP_ID==0x54))
		{
			...
			pbt_buf = CTPM_FW_TRULY_NT;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TRULY_NT));
			if (i_ret != 0)
				dev_err(&client->dev, "%s:upgrade failed. err.\n",
						__func__);
		}	
...
/*********************************************************************************
 wuyu@EXP.BaseDrv.TP, 2015-01-22, add 
 get the last byte in firware file, be able to check whether it is TRULY or OFILM
**********************************************************************************/
u8 fts_ctpm_get_i_file_tpid(void)
{
	u16 ui_sz;
	//ui_sz = sizeof(CTPM_FW);
	printk("get_i_file_tpid_ tp_choose = 0x%x\n", tp_choose);
	if(tp_choose == 0)
	{
		ui_sz = sizeof(CTPM_FW_OFILM);
		if (ui_sz > 2)
		   return CTPM_FW_OFILM[ui_sz - 1];
	}
	if(tp_choose == 1)
	{
		ui_sz = sizeof(CTPM_FW_TRULY);
		if (ui_sz > 2)
	  		return CTPM_FW_TRULY[ui_sz - 1];
	}
	if(tp_choose == 2)
	{
		ui_sz = sizeof(CTPM_FW_BIEL);
		if (ui_sz > 2)
	  		return CTPM_FW_BIEL[ui_sz - 1];
	}
	if(tp_choose == 3)
	{
		ui_sz = sizeof(CTPM_FW_TRULY_NT);
		if (ui_sz > 2)
	  		return CTPM_FW_TRULY_NT[ui_sz - 1];
	}
...
u8 fts_ctpm_get_i_file_ver(void)
{
	...
	if(tp_choose == 3)
	{
		...
		{
		    if(fts_updateinfo_curr.CHIP_ID==0x36)
	            return CTPM_FW_TRULY_NT[0x10a];
		    else
				return CTPM_FW_TRULY_NT[ui_sz - 2];
		}
	}
return 0x00;	/*default value */
}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("fts_choose_tp = 0x%x\n", uc_reg_value);
	printk(" ..15131..TP_FW = 0x%x\n",TP_FW);
	if (/*the firmware in touch panel maybe corrupted */
		uc_tp_fm_ver == FTS_REG_FW_VER ||
		/*the firmware in host flash is new, need upgrade */
	    uc_tp_fm_ver != uc_host_fm_ver ||uc_reg_value != fts_ctpm_get_i_file_tpid())	
	{
		msleep(100);
		printk(" ..........................................enter update..................................\n");
		dev_err(&client->dev, "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
		if (i_ret == 0){
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			printk("[FTS] after update:new version 0x%x\n",uc_host_fm_ver);
			//dev_err(&client->dev, "[FTS] after update:new version 0x%x\n",uc_host_fm_ver);
		} else {
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	if(tp_choose ==0)
		{	
			fw_filenth = sizeof(CTPM_FW_OFILM);
			if(CTPM_FW_OFILM[fw_filenth-12] == 30)
			{
				is_5336_fwsize_30 = 1;
			}
			else 
	...
	if(tp_choose ==1)
		{	
			fw_filenth = sizeof(CTPM_FW_TRULY);
			if(CTPM_FW_TRULY[fw_filenth-12] == 30)
			{
				is_5336_fwsize_30 = 1;
			}
			else 
	...
	if(tp_choose ==2)
	{	
		fw_filenth = sizeof(CTPM_FW_BIEL);
		if(CTPM_FW_BIEL[fw_filenth-12] == 30)
		{
			is_5336_fwsize_30 = 1;
		}
		else 
	...
	if(tp_choose ==3)
	{	
		fw_filenth = sizeof(CTPM_FW_TRULY_NT);
		if(CTPM_FW_TRULY_NT[fw_filenth-12] == 30)
		{
			is_5336_fwsize_30 = 1;
		}
		else 
...
static int fts_debug_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	//struct i2c_client *client = (struct i2c_client *)fts_proc_entry->data;
	struct i2c_client *client = (struct i2c_client *)ft_client;
	...
	//sscanf(writebuf, "%d", &write_data);
	proc_operate_mode = writebuf[0];
	//proc_operate_mode = write_data;
	//dev_err(&client->dev, "%s:write_data=%d\n", __func__,write_data);
	dev_err(&client->dev, "%s:proc_operate_mode=%d\n", __func__,proc_operate_mode);
	...
	case PROC_WRITE_DATA:
		writelen = count - 1;
		dev_err(&client->dev, "%s:writelen=%d\n", __func__,writelen);
		dev_err(&client->dev, "%s:writebuf=0x%02x\n", __func__,(int)(*(writebuf + 1)));
...
static int fts_debug_read( struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	//struct i2c_client *client = (struct i2c_client *)fts_proc_entry->data;
	struct i2c_client *client = (struct i2c_client *)ft_client;
	int ret = 0;
	...
	case PROC_READ_DATA:
		readlen = count;
		dev_err(&client->dev, "%s:readlen=%d\n", __func__,readlen);
		ret = fts_i2c_Read(client, NULL, 0, buf, readlen);
		if (ret < 0) {
			dev_err(&client->dev, "%s:read iic error\n", __func__);
			return ret;
		}
		dev_err(&client->dev, "%s:buf=0x%02x\n", __func__,(int)(buf[0]));
	...
	dev_err(&client->dev, "%s:222buf=%s\n", __func__,buf);
	memcpy(page, buf, num_read_chars);
	dev_err(&client->dev, "%s:page=%s\n", __func__,page);
	ret = simple_read_from_buffer(user_buf, count, ppos, page,num_read_chars);
	ret=copy_to_user(user_buf,buf,num_read_chars);
	//ret = simple_read_from_buffer(user_buf, count, ppos, buf,num_read_chars);
...
{
	fts_proc_entry = proc_create(PROC_NAME, 0777, NULL,&ft5x0x_debug_fops);
