...
#ifdef VENDOR_EDIT
//Jiemin.Zhu@Swap.Android.Kernel, 2015-03-20, modify for orphan task
...
#ifdef VENDOR_EDIT
//Jiemin.Zhu@Swdp.Android.Performance.Memory, 2015/12/29, add for debug memleak
void print_all_process_tasksize(void)
{
    ...
    for_each_process(tsk) {
        ...
        if (test_tsk_thread_flag(p, TIF_MEMDIE)) {
            lowmem_print(2, "###Process(TIF_MEMDIE) %d (%s), adj %d, size %d\n",
...
#ifdef VENDOR_EDIT
//Jiemin.Zhu@Swdp.Android.Performance.Memory, 2015/12/29, add for debug memleak
    static int kill_foreground_times = 0;
#endif /* VENDOR_EDIT */
    ...
    */
    ...
    for_each_process(tsk) {
        ...
        //mtk71029 update for UNINTERRUPTIBLE process
...
*/
        ...
        //mtk71029 add for resolve kill coredump process, it may continue long time.
        if (p->signal != NULL && (p->signal->flags & SIGNAL_GROUP_COREDUMP) ){
            /*lowmem_print(1, "lowmem_shrink filter process: %d (%s) is in middle of coredump, ignore it.", p->pid, p->comm);*/
            task_unlock(p);
...
#ifdef VENDOR_EDIT
//Jiemin.Zhu@Swdp.Android.Performance.Memory, 2015/12/29, add for debug memleak
    if (kill_foreground_times > 10) {
        kill_foreground_times = 0;
