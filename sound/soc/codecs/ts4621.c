...
** File: - ts4621.c
** VENDOR_EDIT
** Copyright (C), 2015-2018, OPPO Mobile Comm Corp., Ltd
**
** Description:
**     audio driver for ts4621
** Version: 1.0
** --------------------------- Revision History: --------------------------------
**  <author>                 <date>               <desc>
** Jianfeng.Qiu@Swdp.Multimedia  2016/04/18  add this file
** 
************************************************************************************/
...
/* Use Device Tree Config or Not */
#if defined(CONFIG_MTK_SOUND)
//Noting
#else
#define CONFIG_OF_FOR_EXTCODEC
#endif

#ifdef CONFIG_OF_FOR_EXTCODEC
//Noting
#else
#define TS4621_I2C_BUS_NO (0)
#define ECODEC_SLAVE_ADDR_WRITE 0xC0
#define ECODEC_SLAVE_ADDR_READ  0xC1
static struct i2c_board_info __initdata  ts4621_dev = {I2C_BOARD_INFO(NAME, (ECODEC_SLAVE_ADDR_WRITE >> 1))};
#endif

static struct i2c_client *ts4621_client;
static int DEFAULT_GAIN = 0x38;
u8 ts4621_global_gain_value = 0x38;
...
static int ts4621_i2c_read(u8 reg, u8 *value)
{
    ...
    do {
        err = i2c_transfer(ts4621_client->adapter, msgs, ARRAY_SIZE(msgs));
...
static int ts4621_init_reg(void)
{
    ...
    iResult |= ts4621_reg_write(0x02, 0xC0);//mute and set volume -60dB
    iResult |= ts4621_reg_write(0x03, 0x00);//clear flag
    ts4621_global_gain_value = DEFAULT_GAIN;
...
static int  ts4621_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    int err = 0;
...
void  ts4621_amp_on(int on, u8 gain)
{
    if (on) {
        ...
        iResult |= ts4621_reg_write(0x02, gain & 0x3E);
        if (iResult) {
...
void  ts4621_amp_on_hph_speaker(int on, u8 gain)
{
    if (on) {
        ...
        iResult |= ts4621_i2c_write(0x02, gain & 0x3E);
        if (iResult) {
            pr_err("ts4621_amp_on 1 Failed");
            return;
        }
...
{
    int iResult = 0;
    iResult = ts4621_reg_read(0x02, value);
    *value = *value & 0x3E;
}
...
static int ts4621_remove(struct i2c_client *client)
{
    return 0;
}
...
static int __init ts4621_init(void)
{
    ...
    //Do nothing
#else
    ret = i2c_register_board_info(TS4621_I2C_BUS_NO, &ts4621_dev, 1);
    if (ret) {
        pr_err("%s: *** i2c board register fail ***\n", __func__);
    }
#endif

    ret =  i2c_add_driver(&ts4621_driver);
    return ret;
}
